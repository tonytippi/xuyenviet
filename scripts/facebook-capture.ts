...
import { admitArtifact, admitArtifactAlias, assertCaptureCacheReady, findArtifactByAlias, findForceLiveArtifact, findReusableArtifact, finishImport, linkForceLiveArtifact, prepareImport, supersedeDefaultArtifacts } from "@xuyenviet/worker-domain/features/knowledge/capture-cache";
import { flushCachedArtifact } from "@xuyenviet/worker-domain/features/knowledge/capture-orchestration";
import { CAPTURE_PAYLOAD_SCHEMA_VERSION, FACEBOOK_CAPTURE_METHOD_VERSION, canonicalizeFacebookUrl, captureReuseKey, facebookResourceIdentity } from "@xuyenviet/worker-domain/features/knowledge/capture-identity";
import { findFacebookCaptureImportByCorrelationToken, listQueuedFacebookSources, normalizeDiscoveredFacebookPosts, recordFacebookCaptureFailure, updateQueuedFacebookSourceRawText, type SafeFacebookCaptureMetadata } from "@xuyenviet/worker-domain/features/knowledge/facebook-capture";
...
export async function extractVisibleFacebookText(page: Page, finalUrl: string): Promise<ExtractedFacebookText> {
  const result = await page.evaluate(String.raw`(() => {
    ...
    const chooseText = (element) => {
      ...
      return { text: innerText, source: "innerText", innerTextLength: innerText.length, textContentLength: normalizeText(element.textContent ?? "").length, htmlTextLength: 0 };
    };
    ...
    const dialogCandidates = dialogCandidateSets.flatMap((set) => set.candidates);
    const pageCandidateSet = collectCandidates(document, "page");
    const selectedSet = dialogCandidates.length > 0 ? { candidates: dialogCandidates } : pageCandidateSet;
    ...
    const targetCandidates = targetPostRoot ? collectCandidates(targetPostRoot, "target").candidates : [];
    const best = targetPostRoot ? targetCandidates[0] : candidates[0];
    ...
    return {
      ...
      diagnostics: {
        ...
        dialogCandidateCount: dialogCandidates.length,
        ...
        candidateCount: targetPostRoot ? targetCandidates.length : candidates.length,
    ...
    for (const source of queued) {
      ...
      try {
        ...
        if (artifact) {
          ...
          if (!options.yes && !(await confirmSave(source.sourceId, payload.rawText))) {
