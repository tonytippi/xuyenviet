...
type CapturedYoutubeSegment = YoutubeSegment & { reuseKey: string; resourceIdentity: string; captureMethodVersion: string; capturedAt: string; metadata: ReturnType<typeof sanitizeYoutubeMetadata> };
    ...
    for (const source of queued) {
      ...
      try {
        ...
        if (cached) {
          ...
          if (!options.yes && !(await confirm(source.sourceId, payload.evidence.length))) {
            console.log(`${source.sourceId}: finished skipped (${formatDuration(startedAt)})`);
            continue;
          }
          stage = "title_lookup";
        ...
        const capturedSegments: CapturedYoutubeSegment[] = [];
        ...
        for (const [index, window] of windows.entries()) {
          ...
          capturedSegments.push({ evidence: result.evidence, window, latencyMs: result.latencyMs, reuseKey: segmentReuseKey, resourceIdentity: segmentResourceIdentity, captureMethodVersion: segmentCaptureMethodVersion, capturedAt, metadata: sanitizeYoutubeMetadata({ captureMethod: "gemini_youtube_url", capturedAt, sourceUrl: url, model, mediaResolution, promptVersion: youtubeEvidencePromptVersion, evidenceCount: result.evidence.length, latencyMs: result.latencyMs, promptTokens: result.usage?.promptTokenCount, outputTokens: result.usage?.candidatesTokenCount, totalTokens: result.usage?.totalTokenCount, videoDurationSeconds: durationSeconds, windowStartSeconds: window.startOffsetSeconds, windowEndSeconds: window.endOffsetSeconds }) });
        ...
        for (const segment of capturedSegments) {
          await admitArtifact(cacheClient, { provider: "youtube", reuseKey: segment.reuseKey, resourceIdentity: segment.resourceIdentity, captureMethodVersion: segment.captureMethodVersion, payloadSchemaVersion: YOUTUBE_CAPTURE_PAYLOAD_SCHEMA_VERSION, promptVersion: youtubeEvidencePromptVersion, model, payload: { evidence: segment.evidence, window: segment.window, metadata: segment.metadata }, metadata: { captureOrigin: "live" }, capturedAt: segment.capturedAt });
        }
        stage = "title_lookup";
        const title = await requestYoutubeTitle(url);
        const capturedAt = new Date().toISOString();
