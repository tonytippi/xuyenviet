...
Rule: Search captures immutable provider results. Retrieval records the selected result's fact scope, capture identity, time, and requirement binding inside the bounded answer decision snapshot and provenance; it creates no projection table. A result with unresolved or mismatched geography cannot become a factual premise, satisfy a required need, or authorize a different fact from the same result.
...
Rule: Deleting an ordinary chat removes its planning session, retrieval snapshots/provenance, recommendation state, derived context, and embeddings, but cannot mutate an unrelated Trip plan. Deleting a primary conversation requires an owner-scoped replacement or deletion of the owning Trip Project. Deleting a Trip removes its snapshots, canonical-path references, proposals, and derived context; retained audit metadata must not reconstruct traveler content.
...
Rule: `acceptTripCreationRecommendation(...)` is an explicit, idempotent Chat/Trips command. It revalidates owner, recommendation, current flat planning session, and deletion/version fences before atomically creating the Trip Project, its primary conversation, and an initial pending proposal under existing aggregate rules. It never turns ambiguous, assumed, or unconfirmed values into plan state without the existing proposal Apply path.
...
Rule: Until route coverage needs runtime editing, Retrieval owns one typed, code-reviewed route registry/coverage manifest in the database package. Its explicit version is pinned in decisions and Trip path references. Deployment activates the code version; invalid manifests fail unit/build verification and never require registry tables, a publisher, a Worker operation, or runtime CAS. A future runtime-editing requirement requires a new architecture decision.
...
Rule: Each answer decision pins planning mode, Trip/proposal references, normalized-question digest, requirement-profile version, registry/coverage version, selector version, and exact rendered handles. `usedInPrompt` derives only from those rendered handles; `citedInAnswer` remains a separate same-turn validated fact.

Rule: Normal production stores bounded selected/rendered decisions, exact requirement outcomes, bounded top rejection reasons, and aggregate excluded counts. Full candidate traces exist only in synthetic fixtures or time-bounded diagnostics, never a durable evaluation-run subsystem. Traveler-derived payloads inherit chat/Trip deletion and retention rules.

Rule: Retrieval writes one bounded JSON decision snapshot into the existing `assistant_retrieval_decisions` aggregate for each completed answer. That snapshot contains requirement keys/outcomes, selected fact handles, excluded counts/reasons, web query/scope decisions, and the exact rendered handles; separate normalized run, contribution, selection, query-plan, or read-policy tables are forbidden for this clean-break tranche. AI Orchestration owns `finalizeAiAnswer(...)` coordination and answer provenance. In one PostgreSQL transaction Chat/Trips inserts the assistant message, Retrieval inserts the decision snapshot, Usage appends the usage event, and AI Orchestration writes provenance. Provider failure appends failure Usage without a completed message or decision; the existing AI Ask command/idempotency fence prevents a second terminal result.

Rule: Requirement-key identity is a deterministic digest of the code-owned intent-profile version and canonical facet, importance, scope/leg, constraint, and freshness fields. The pure evaluator creates in-memory `knowledge | web` contributions with exact fact identity, owner/capture revisions, eligibility/scope/freshness decisions, and permitted render variants, then serializes only the bounded final decision snapshot. A changed field or generation rule changes the code-owned version; selector, renderer, provenance, and tests consume the same types.

Rule: Replayable web execution constructs a bounded minimized-query value binding requirement keys, allowed scope terms, excluded private-context classes, provider-policy version, and request digest before Search is called. Every selected web fact pins its capture/result identity, text digest, scope decision, and requirement key inside the final retrieval-decision snapshot and provenance. Replaying the same inputs reproduces the same pure scope decision without normalized projection tables.

### AD-37: Release Safety Is Executable And Proportional

Binds: deterministic fixtures, focused unit/integration verification, disposable-target reset/reseed, and optional experiments.

Prevents: release ceremony becoming a second product, production-style control planes being built before production data exists, or critical safety behavior shipping without executable proof.

Rule: Epic 21 release readiness is code-owned and executable. Canonical CLAR, PM, RP, RN, WS, DEL, TC, and clean-break fixtures cover hard-off-route contribution, unrelated-need satisfaction, hypothetical/pending-as-committed behavior, private Trip leakage, silent required-gap omission, provider failure, deletion, and conversion safety. Unit and serial PostgreSQL integration tests must pass with literal-zero violations for those critical cases; no database gate-profile, cohort-membership, report, approval, or evidence-window tables are introduced.

Rule: The target database is explicitly disposable before Epic 21 implementation. The existing guarded reset command validates target identity and no-runtime-overlap confirmation, then migration/reseed establishes the only supported schema. If a non-disposable target or durable shared/customer data is discovered, work stops and requires an expand-migrate-contract architecture update.

Rule: Optional retrieval experiments such as FTS, embeddings, RRF, reranking, or AI adjudication remain separately justified. The deterministic code-owned lexical path ships first; an experiment cannot add runtime policy tables or fallback complexity without a concrete approved requirement.

### AD-38: Required-Need Retrieval Replaces Card Count In One Clean Break

Binds: the historical fewer-than-three behavior, target schema, required-need activation, reset/reseed, and executable compatibility tests.

Prevents: Story 4.5's historical card-count rule surviving as product behavior or a nonexistent migration risk creating shadow/read-policy/cutover infrastructure.

Rule: Epic 21 removes the fewer-than-three branch, target-count configuration, and active runbook authority in the same clean-break deployment that makes AD-34 required-need coverage authoritative. Existing count columns may remain inert telemetry so the one migration does not couple storage foundation to later retrieval behavior. There is no `legacy | v6_shadow | v6_active` runtime mode, paired execution, would-render manifest, read-policy row, cutover record, approval row, or legacy rollback target.

Rule: `COMP-01` and `COMP-02` remain executable behavioral tests: one exact sufficient contribution does not trigger web work solely because its count is low, and multiple irrelevant contributions cannot hide a required gap. Repository reference checks prove no executable card-count authority remains.

Rule: Recovery before real shared/customer data exists is code rollback followed by the same guarded reset/reseed. Once durable data exists, destructive recovery is forbidden by NFR-16 and any later retrieval replacement requires a new migration/release decision.

### AD-39: Material Planning Context Uses One Flat Bounded Session

Binds: intent-specific material slots, explicit traveler answers, one-at-a-time clarification, session revision, and entry to retrieval.

Prevents: silent defaults, repeated questions, cross-scope leakage, stale extraction overwrite, autonomous clarification loops, and a generic planning workflow becoming a second product.

Rule: Epic 21 adds one conversation-owned `planning_context_sessions` row containing a bounded JSON document: intent, flat typed slots with journey/leg/place/stay scope, missing slot keys, `collecting | ready | superseded` status, source message IDs, and revision. Intent profiles are small typed constants beside the reducer. There are no graph, node, deliverable-instance, claim, plan-attempt, extraction-attempt, assumption, or policy tables.

Rule: One existing extraction capability may propose explicit values for the newest traveler message. A deterministic reducer validates declared keys/scopes/value shapes, preserves compatible slots, turns a same-scope contradiction back into a missing slot, and computes the next material question. It asks at most one question per blocked turn. Model output cannot declare readiness, create route authority, or mutate Trip state.

Rule: The existing AI Ask command fence owns retry and terminalization. A blocked turn records one clarification message and Usage and creates no retrieval, web, main-answer, annotation, recommendation, or proposal effect. Owner, conversation, source-message, and expected session revision checks reject stale or deleted work. Missing model, timeout, or invalid output persists safe retry guidance and never falls through to the answer model.

Rule: A ready session is a current-answer input only. Applied Trip state remains authoritative under AD-29/AD-30. Session values can reach durable Trip state only through the existing pending-proposal and owner Apply boundary.

### AD-40: Chat-To-Trip Reuses The Existing Recommendation Aggregate

Binds: conversion eligibility, explicit dismissal, idempotent accept, pending proposal creation, and deletion.

Prevents: treating non-click as decline, converting stale or ambiguous context, copying a transcript into a Trip, applying values before review, duplicate Trip creation, and a generic opportunity workflow.

Rule: The existing recommendation aggregate exposes only `eligible | accepted | dismissed | invalidated`. A completed unscoped answer is eligible when the current planning session has at least one explicit supported value that maps to an existing Trip proposal operation. Ambiguous, missing, stale, unsupported, and assumption-only values are ineligible. A later completed turn recomputes current eligibility; no manifest history, suspension state, policy catalog, or background refresh job exists.

Rule: `acceptTripCreationRecommendation(...)` remains the sole conversion command and trusts no client context snapshot. Under its existing owner/conversation lock and idempotency key, it rechecks current session and deletion/version fences, maps supported explicit values to existing database proposal operations, validates them through the database proposal command, and atomically creates one Trip, its separate primary conversation, and one pending proposal. The existing owner Apply command is the only path that changes Trip state.

Rule: The original conversation stays separate. Transcript, assistant prose, prompts, provider payloads, model reasoning, ambiguous values, and assumptions are not copied. Explicit dismissal updates the existing recommendation; non-click does nothing; `continueInTrip(...)` changes URL scope only. Conversation/Trip deletion invalidates related recommendation and replay state through existing cascades or the smallest proven explicit cleanup. No opportunity/manifest table, workflow engine, new endpoint, service, Worker, or environment flag is introduced.
...
- `trip_projects`, `conversations`, `messages`, one `planning_context_sessions` bounded JSON document per active conversation, existing `trip_recommendation_decisions` using four simple conversion states, `assistant_response_provenance`
...
- code-owned versioned planning profiles, required-need definitions, and route registry/coverage manifest; these are not database entities in Epic 21
- existing `assistant_retrieval_decisions` extended with one bounded JSON decision snapshot; existing `web_search_results`, `assistant_response_provenance`, `ai_gateway_models`, `ai_usage_events`, `feedback`, `eval_runs`, and `audit_events`
...
Retrieval implements AD-17 and AD-34 through AD-39 through the minimal flow in [retrieval-trip-aware-solution-design.md](retrieval-trip-aware-solution-design.md). The source bundle contains the current flat planning session, pinned planning mode/Trip references, selected active knowledge facts, eligible external web facts, and explicit required-need outcomes. Traveler prompts contain only traveler-safe bounded snapshots; they exclude raw source material, copied post bodies, image-analysis notes, operator-only evidence, provider payloads, and admin-only metadata.
...
The five PRD beta prompts remain the initial product-quality set. Epic 21 adds executable fixtures for required-need coverage, route applicability, planning-mode isolation, canonical Trip path behavior, scoped web use, provider failure, capacity pressure, deletion, conversion, and clean-break card-count removal. [retrieval-trip-aware/evaluation-and-release-gates.md](retrieval-trip-aware/evaluation-and-release-gates.md) defines this verification checklist under AD-37 and owns no runtime gate lifecycle.
...
- Code-owned planning/retrieval/route versions, guarded disposable-target reset/reseed, and no Epic 21 runtime release-control tables or Worker operation.
