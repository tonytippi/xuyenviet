...
Rule: The aggregate distinguishes stable plan state from conversational context. Anchors, dated legs, activities, manually confirmed accommodations, alternatives, item state, and owner-maintained trip constraints are structured Trip Planning records. `chat_context` remains extracted conversational/trip facts and never becomes an alternative itinerary writer or source of truth.
...
Rule: `trip_project_constraints` is one owner-scoped, versioned structured record per Trip Project. It holds only travel-relevant travelers/children, vehicle or EV needs, driving tolerance, budget range, preferences, and avoid-list values. Chat extraction may propose a constraint update, but only an owner-confirmed Chat/Trips command writes it; a constraint correction participates in the Trip Project aggregate version and deletion lifecycle.
