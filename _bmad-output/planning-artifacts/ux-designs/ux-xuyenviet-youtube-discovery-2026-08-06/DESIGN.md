...
- **Candidate inspector** uses `{components.discovery-inspector}`. It shows canonical URL, safe video/channel metadata, query/reason, recommendation, 3-5 factors and penalties, derived comment signals, prior safe capture outcome, and actions `Accept`, `Để sau`, `Bỏ qua`.
