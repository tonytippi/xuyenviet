...
| Candidate review workspace | Candidate item / Mission candidate list | Ranked queue plus selected inspector for one-at-a-time Accept, defer, or skip decisions. |
...
| Automation Health | Discovery navigation / incident drill-in | See schedule/enabled state, throughput/review backlog, incidents, usage telemetry, data freshness, and affected safe records. |
...
| Candidate queue | Candidate review workspace | Paginated or explicit load-more ranked list. One selected candidate at a time. Rows show safe scan metadata and never use raw comments or model text. After an action, refetch active results and select the first remaining eligible candidate; otherwise show completion state. |
| Candidate inspector | Candidate review workspace | Shows safe candidate context, recommendation, concise factors/penalties, derived comment signals, and prior safe capture outcome. Numeric scores are progressive disclosure for authorized operators only. |
...
- Queue rows expose title, recommendation, priority, and selected state to screen readers.
...
- Candidate factors, channel metrics, comments, and AI-triage output are ranking context only. UI must not present them as verified facts, evidence, or publication approval.
