...
- The workspace has `apps/web`, `apps/admin`, `apps/api`, and `apps/worker` applications with narrowly extracted `database`, `domain`, `contracts`, and `config` packages. Presentation code belongs to its application workspace; root server compatibility code moves only when it needs a shared consumer.
