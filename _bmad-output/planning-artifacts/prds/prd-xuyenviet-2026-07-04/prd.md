...
- FR-24D: The system shall give each discovered candidate an independent, auditable processing result and immutable AI disposition, and shall complete a source ingestion only after discovery and its candidate work have completed or failed. A source may complete successfully with mixed candidate dispositions or when no candidate is applied.
...
- FR-25B: The system shall support random quality sampling of active claims so operators can measure false-positive publication without delaying the normal ingestion flow. The initial sampling rate shall be 15% for the first four weeks, and `needs_operator` outcomes retain a separate immutable sampling obligation. Sampling is quality monitoring rather than publication approval and may be open for an active card version. When a quality-sampling review identifies a high-severity publication failure, the system shall record the disposition and apply an auditable, cohort-scoped containment action through the knowledge lifecycle command. Unrelated cohorts shall remain unchanged.
...
- NFR-16: Use a local disposable reset only under the repository reset safeguards. Durable or overlapping runtimes require an approved forward-only expand-migrate-contract plan, a bounded idempotent backfill where needed, one selected traffic writer, `dualWrite: false`, and rollback by compatible binary rather than destructive schema rollback.
...
- AC-9F: A completed ingestion job can contain mixed candidate dispositions without its technical status misrepresenting those outcomes. An operator resolution does not change a completed candidate's AI disposition.
- AC-9G: No active card has open primary operator work; each card version has at most one open primary work item and one open sampling item; a stale or superseded work item cannot mutate a card, evidence, audit, or search projection.
