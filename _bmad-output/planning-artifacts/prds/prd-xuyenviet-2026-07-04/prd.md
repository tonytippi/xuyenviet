...
- NFR-16: The current lifecycle normalization uses a clean-break migration, reset, and reseed only under repository safeguards because all current targets are disposable. If durable shared or customer data exists before it ships, implementation shall stop and use an approved forward-only expand-migrate-contract plan instead.
