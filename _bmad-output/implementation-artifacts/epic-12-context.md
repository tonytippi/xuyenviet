...
Establish a separately deployable worker and migration runtime that executes durable background work safely and observably, so request-serving processes never claim jobs and staging/public releases cannot run workloads against incompatible schemas or silently lose work during cutover.
...
- Liveness proves the process can run. Readiness proves assigned configuration, database, schema compatibility, and critical dependencies are valid; worker readiness additionally proves its loop state. On shutdown, workers stop taking new claims and safely complete or release in-flight work through persisted lease rules.
...
- A worker/API/web/admin workload must not become ready, receive traffic, or claim work unless the deployed schema version is within its declared compatibility range. The migration job runs once under its release lock and succeeds before dependent workloads receive traffic or work.
...
- Clean-break migrations are allowed only when the target data is confirmed disposable and no runtime overlap exists. For durable data or overlapping runtime versions, require an approved expand-migrate-contract compatibility matrix and migration-job gate; rollback changes routing or compatible code, never destructively rolls back persisted schema.
...
- Story 12.2 depends on the worker runtime and validates compatibility admission, telemetry, and loop-cutover evidence before legacy loops can be retired.
- Story 12.3 supplies the compatibility and migration gates that all independently deployed workloads, including the worker validated in Story 12.2, must obey during durable-data or overlapping-runtime releases.
