...
Knowledge cards must retain their required travel metadata, fixed taxonomy, confidence label, freshness indicator, provenance, conditions, and an exact bounded evidence quote/span before becoming eligible. AI can propose a quote but never owns its offsets: Knowledge resolves only unique Unicode-normalized quote matches and persists the exact raw capture substring with a system-derived code-point span. Traveler-visible facts and evidence must not retain or expose PII, sensitive content, raw material, private source data, provider payloads, or operator-only fields.
