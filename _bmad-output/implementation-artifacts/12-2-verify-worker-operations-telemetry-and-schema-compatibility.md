...
- [x] [Review][Patch] Classify persisted Worker outcomes truthfully [packages/worker-domain/src/features/knowledge/extraction-jobs.ts:125] — terminal extraction failures are emitted as `retry`; ingestion reports terminal candidate failure as `success` and retryable candidate work as `contended`; indexing reports exhausted or lost-lease work as `retry`; and a fenced-out outbox claim remains `success` at `packages/worker-domain/src/features/ai/domain-outbox-worker.ts:87`. Map the persisted/fenced outcome to `failure`, `retry`, or `contended` before emitting the compiled-adapter telemetry event, preserving each existing guarded durable transition.
- [x] [Review][Patch] Emit recovery telemetry per extraction and ingestion record [packages/worker-domain/src/features/knowledge/extraction-jobs.ts:117] — a terminal stale recovery overrides the result code for a separately claimed record and attaches that claimed record's durable ID. Return/emit recovered records individually, retaining each claimed record's own persisted result, and cover mixed recovery-plus-success and recovery-plus-retry polls. The same defect exists in `packages/worker-domain/src/features/knowledge/ingestion-worker.ts:8`.
...
- 2026-08-01 final telemetry closure repair: Worker observations now preserve every guarded persisted disposition: extraction retry/terminal/recovery and lost completion, ingestion primary/candidate retry/terminal/recovery, indexing retry/exhaustion/lost claims, and outbox fenced/lost failure transitions. Retry is emitted only after durable requeue, failure only after terminalization, and contended only after a lost or fenced claim. Serial `DATABASE_URL_TEST` matrix passed: `tests/worker-adapter-boundary.test.ts`, `tests/knowledge-ingestion-jobs.test.ts`, `tests/knowledge-ingestion-pipeline.test.ts`, `tests/knowledge-extraction-worker.test.ts`, `tests/knowledge-indexing-worker.test.ts`, `tests/domain-outbox.test.ts`, and `tests/operational-telemetry.test.ts` (7 files, 125 tests). `pnpm typecheck` passed; `pnpm lint` had 0 errors and 5 pre-existing warnings; `git diff --check` passed. Final synchronous review found no actionable correctness issues. No Story 12.3 code changed.
- 2026-08-01 final Epic 12 closure repair: multi-marker indexing batches now emit per-marker observations rather than pairing an aggregate disposition with the first claim's metadata. Recovery is evaluated once before batch claims; each recovered marker emits its own retry/failure, durable ID, retry count, and recovery disposition, while claimed markers retain their own success/retry/failure/contended facts. Serial `pnpm vitest run tests/knowledge-indexing-worker.test.ts tests/operational-telemetry.test.ts tests/worker-adapter-boundary.test.ts --maxWorkers=1 --no-file-parallelism` passed (3 files, 29 tests). `pnpm typecheck` passed; `pnpm lint` had 0 errors and 5 pre-existing warnings; `git diff --check` passed. Final synchronous review found no actionable findings. No Story 12.3 code changed.
- 2026-08-01 final closure recovery repair: extraction and ingestion now emit one recovery observation for every durable recovered row before independently emitting each newly claimed row's persisted disposition and durable ID. Mixed stale-terminal-plus-new-retry extraction and exhausted-plus-new-terminal ingestion regressions prove recovery cannot overwrite a separately claimed record. The compiled adapter regression verifies the same split for a recovered ingestion lease and a new success claim. Serial `pnpm vitest run tests/knowledge-extraction-worker.test.ts tests/knowledge-ingestion-jobs.test.ts tests/operational-telemetry.test.ts tests/worker-adapter-boundary.test.ts --maxWorkers=1 --no-file-parallelism` passed (4 files, 52 tests). `pnpm typecheck` and `git diff --check` passed; `pnpm lint` had 0 errors and 5 pre-existing warnings. Final synchronous review found no actionable findings. No Story 12.3 code changed.
...
- `packages/worker-domain/src/features/knowledge/extraction-jobs.ts`
