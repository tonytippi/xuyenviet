...
2. **Given** one enabled query run calls documented YouTube Data API search, **when** it receives multiple distinct eligible individual-video results, **then** it creates or updates one canonical Discovery candidate per distinct video ID.
   - It records a separate query/run appearance for every distinct eligible result, so one query can yield many candidates and one candidate can retain many appearances.
...
- [x] Establish one shared canonical YouTube video URL contract (AC: 1)
   - [x] Add an explicit exported canonicalizer in the existing shared domain boundary, returning a typed `{ videoId, canonicalUrl }` result or `null`; it must have no database, Worker, provider, or capture dependency.
   - [x] Accept only HTTPS individual-video URLs on `youtube.com`, a `*.youtube.com` host explicitly supported by the existing intake contract, or `youtu.be`; reject credentials, ports, fragments, channel/playlist/shorts/embed/live paths, extra path segments, malformed percent-encoding, and unsupported hosts before any external call.
   - [x] Preserve the established `^[A-Za-z0-9_-]{6,20}$` video-ID grammar and normalize every accepted input to exactly `https://www.youtube.com/watch?v=<video-id>` without tracking parameters.
   - [x] Replace the local YouTube normalization in `packages/database/src/admin-knowledge-intake.ts`; migrate the capture/seed/capture-identity helpers only where they can delegate to the same contract without pulling database/provider code into the shared boundary. Do not leave a second acceptance grammar.
   - [x] Preserve non-YouTube intake normalization exactly. `normalizeIntakeUrl(...)` must continue returning the existing safe hostname/kind shape for Facebook and generic URLs.

- [x] Add the minimal Discovery-owned candidate graph (AC: 2-4)
   - [x] Add closed schema values and tables only for: canonical candidate, query/run appearance, and bounded ranking-history records. Use `youtube_discovery_*` names and Drizzle as the sole schema/migration authority.
  - [ ] Candidate identity is the canonical video ID, protected by a database unique constraint. Persist the canonical URL and only bounded safe discovery fields required now: identity, creation/update timestamps, and explicitly bounded state needed by later triage/review. Do not add a Knowledge source ID, acceptance state, source lifecycle field, raw provider JSON, comments, transcripts, media, prompt/response, credentials, cookies, or arbitrary JSON.
   - [ ] Each appearance references exactly one candidate, one Discovery run, and the query proposal that owns that run. Candidate persistence derives the proposal from the locked run and never accepts a caller-supplied proposal ID. A provider-stage run must have a non-null enabled proposal and persisted query text. Enforce proposal/run consistency by a composite constraint or by storing only `runId` and deriving the proposal through the run. Preserve a safe result ordinal and discovery timestamp. Collapse repeated video IDs within one provider response to one appearance per `(run, candidate)`; preserve separate appearances when the candidate occurs in distinct runs.
   - [ ] Ranking history references candidate, optional appearance where context exists, run, policy version, timestamp, and a closed stage: `discovered` in this story, with `enriched` and `triaged` reserved for Stories 18.5 and 19. The Story 18.4 `discovered` record has no free-form payload and retains only those relation/timestamp/stage fields; no arbitrary JSON, model-derived recommendation, score, raw response, comment, provider payload, or Knowledge link belongs in this story. Keep at most 20 ranking-history rows per candidate; on insert, evict that candidate's oldest rows in the same transaction.
  - [ ] Add only indexes that serve canonical lookup, run/query appearance lookup, and chronological candidate history. Do not pre-create triage, review-action, channel, comment, retention, blocking, or UI read-model tables; Stories 18.5, 19, and 20 own those concerns.
   - [x] Create the next sequential Drizzle migration after `0048_discovery_operator_priority_and_policy_transition`, update `drizzle/migrations/meta/_journal.json`, and verify the schema/migration match. Preserve all existing migrations unchanged.

- [x] Implement fenced, idempotent candidate persistence (AC: 2-4)
...
- [x] Add a documented, bounded YouTube Data API search stage (AC: 2-3)
  ...
  - [ ] Read the API credential only in the Worker adapter/configuration boundary. It must be required and nonblank, never logged, persisted, returned through telemetry, or included in tests. Reuse the existing project `fetch` stack; do not add an SDK.
   - [ ] Treat provider responses as untrusted: validate exact expected structural fields, retain only valid `youtube#video` IDs that independently pass the shared canonicalizer, and ignore malformed/non-video entries. Collapse duplicate video IDs in one structurally valid response to one candidate/appearance. Do not retain snippets or the provider response in Story 18.4.
   - [ ] Classify network, timeout, non-2xx, and malformed top-level or required-result-structure responses into the existing safe transient retry path without storing remote error text. A structurally valid response that yields zero eligible results after filtering completes successfully with zero candidate writes. Preserve Story 18.2 retry/backoff/exhaustion and later-run independence behavior.
...
- [x] Publish and use a Knowledge-owned prior-capture eligibility port (AC: 5)
   - [ ] Define a narrow domain contract that accepts canonical video identity plus an exact Knowledge-owned durable compatibility key. Before implementing the port, the Knowledge owner must define that key from the current capture compatibility rules and persist every required component on the applicable capture-history representation; it must include capture method/version and payload-schema version, and may include another existing compatibility component only when the Knowledge owner proves it affects reuse. Discovery neither derives nor supplies a free-form version. Return only a closed safe eligibility result, for example `eligible | already_compatible | unavailable`; it must not expose source IDs, source URLs, capture IDs, timestamps, raw metadata, evidence, source state, or failure text.
   - [ ] Implement the port in the Knowledge/database owner using its own tables and existing capture compatibility helpers. Return `already_compatible` only when the persisted durable key exactly matches. Discovery calls the port through composition only; it must not import `sources`, `sourceCaptureVersions`, or any Knowledge repository/table.
   - [x] Consult the safe result before persisting a search candidate. `already_compatible` must not create a reviewable Discovery candidate; `unavailable` must fail closed into the existing safe retry/error path rather than infer eligibility or read Knowledge directly.
   - [x] Bind the production port in `apps/worker/src/adapters.ts` beside the existing owner-published planning ports. Keep the port bounded, abortable if it can wait, and test-injectable without exporting a generic cross-module database reader.

- [x] Verify shared identity, ownership, fences, and safe persistence (AC: 1-5)
  - [ ] Add DB-free unit tests for every allowed URL form and rejection path; ensure canonicalizer equivalence across Knowledge intake, seed URL parsing, capture identity/canonicalization, and Discovery. Cover host case/trailing dot, tracking parameters, credentials, ports, duplicate `v`, extra path, playlists/channels/shorts/embed/live, malformed URLs, and video-ID boundaries.
   - [ ] Add DB-free provider-adapter tests asserting fixed documented request parameters, non-disclosure of API keys/provider body, malformed/non-video result filtering, repeated-ID collapse within a response, valid zero-eligible-result completion, safe transient classification, and no request when a run is already fenced/revoked.
...
- `scripts/youtube-capture.ts` already reads `YOUTUBE_DATA_API_KEY`; use this Worker-only configuration key and do not create a second YouTube credential name or configuration path.
...
- A valid empty or fully filtered result page is a completed search outcome, not a provider failure. Provider result ordinal is the first retained occurrence after within-response video-ID deduplication.
...
- Include database constraints/tests proving an appearance cannot claim a query proposal other than its owning run's proposal, ranking history cannot exceed 20 rows per candidate, and repeated IDs in one response create one appearance while distinct runs retain all appearances.
...
- 2026-08-09: Independent readiness validation identified and resolved guide gaps for within-response deduplication, run/proposal provenance, durable Knowledge-owned capture compatibility, valid empty search completion, bounded ranking history, and Worker provider rollout. Status remains ready-for-dev; no implementation or verification command was run.
...
- 2026-08-09: Implemented canonical identity, durable compatibility columns/port, Discovery candidate graph migration 0049, fenced persistence, native-fetch search, and Worker composition. Unit/typecheck verification recorded below; serial database integration requires `DATABASE_URL_TEST`.
- 2026-08-09: `pnpm typecheck` and `pnpm build` passed. Focused unit selection passed 16 tests. `pnpm test:integration -- --run tests/youtube-discovery-execution.integration.test.ts` migrated the test database but Vitest executed the full integration project and reported baseline failures outside this story; it also exposed corrected strict URL expectations.
- 2026-08-09: Repair: candidate persistence now rechecks the active lease, current enabled policy, and linked enabled proposal immediately before every candidate, appearance, and ranking-history write and once before reporting completion. A disabled guard terminally cancels and records one safe terminal audit; a lost claim is contended. The exact current capture compatibility descriptor is a typed Knowledge-owned contract (`packages/domain/src/knowledge/youtube-capture-compatibility.ts`); Discovery now supplies only video identity to the owner-published eligibility port.
- 2026-08-09: Verification passed: `pnpm exec vitest run --project unit tests/youtube-video.test.ts tests/youtube-discovery-search.test.ts tests/youtube-discovery-ownership.test.ts tests/youtube-discovery-owner-port-composition.test.ts tests/youtube-discovery-execution.test.ts` (5 files, 17 tests); `pnpm exec vitest run --project unit tests/admin-knowledge-intake.test.ts tests/youtube-seed-urls.test.ts tests/youtube-capture.test.ts tests/youtube-video.test.ts tests/youtube-discovery-search.test.ts tests/youtube-discovery-policy.test.ts tests/youtube-discovery-ownership.test.ts tests/youtube-discovery-worker.test.ts tests/youtube-discovery-execution.test.ts tests/youtube-discovery-planning.test.ts tests/youtube-discovery-owner-port-composition.test.ts tests/admin-youtube-discovery-contract.test.ts` (10 files, 57 tests); `pnpm exec vitest run --project integration tests/youtube-discovery-candidates.integration.test.ts` (1 serial file, 5 tests); `pnpm typecheck`; `pnpm lint` (0 errors, 43 pre-existing warnings); `pnpm build`; `git diff --check`. The wrapper form `pnpm test:unit -- --run <files>` is invalid for focused selection because its extra `--` causes Vitest to run the full configured project; `vitest` must receive files directly as above. Full unit run has one pre-existing unrelated failure in `tests/traveler-ui-foundation.test.ts` due to an expected delete-button class string that no longer matches its source.
...
- packages/domain/src/youtube-video.ts
- packages/database/src/knowledge-youtube-capture-eligibility.ts
- packages/database/src/youtube-discovery/index.ts
- packages/database/src/schema.ts
- packages/worker-domain/src/features/youtube-discovery/youtube-search.ts
- drizzle/migrations/0049_discovery_youtube_candidates.sql
- packages/domain/src/knowledge/youtube-capture-compatibility.ts
- tests/youtube-discovery-candidates.integration.test.ts
- packages/database/src/runtime-config.ts
