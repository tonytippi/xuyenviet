...
**Execution:**
- [x] `packages/contracts/src/index.ts`, `packages/domain/src/index.ts`, and `packages/database/src/index.ts` -- strict create/delete/feedback/proposal/annotation contracts and a principal-supplied PostgreSQL port preserve locked proposal transactions, audit/history, expiry, annotation binding, and AI command discard semantics.
- [x] `apps/api/src/conversations/`, `apps/api/src/feedback/`, `apps/api/src/referrals/`, and `apps/api/src/auth/` -- admitted Nest direct endpoints own create/delete/feedback, proposal apply/dismiss, annotation action, and OAuth-bound referral capture; controllers remain transport-only.
- [x] `src/features/ai/direct-api-client.ts`, `src/features/chat-trips/direct-shell-loader.tsx`, `src/features/ai/ai-ask-composer.tsx`, and `src/app/ai-ask/page.tsx` -- traveler reads/commands use relative direct APIs, strict response parsing, bounded workspace projection, post-command reload, and repaired stream/shell behavior.
- [x] `src/auth.ts`, `src/server/auth.ts`, `src/app/api/auth/[...nextauth]/route.ts`, `src/features/auth/actions.ts`, `src/features/chat-trips/actions.ts`, and `src/features/feedback/actions.ts` -- traveler command server-action owner is removed; retained root Auth.js action is the root admin layout path and `apps/admin` remains unchanged.
