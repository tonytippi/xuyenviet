...
- `tests/trip-planning-safety.test.ts` — NEW + UPDATED (patch): cross-cutting adversarial safety suite (16 tests) covering multi-owner existence-leakage, cross-owner commands, primary-conversation invariant (deleted/cross-owner + unlinked schema-FK), concurrent apply/apply + FOR UPDATE contention proof, deadlock-tolerant concurrent apply/dismiss, sequential no-op return values, worker-vs-read expire idempotency, concurrent reorder + held-lock contention, expired proposal apply, project-deletion cascade to history + constraints, and ordering-precondition fail-closed. Adds a scoped multi-connection pool harness and held-lock contention proofs.
...
- `tests/ai-ask-shell.test.ts` — UPDATED: 12 AC3 accessibility/recovery tests (patched in place: reduced-motion asserts no non-essential transitions, unconditional done-handler, plan-history-list aria-live, behavior-level P4/Q3 DOM rendering, comment-free refresh behavior) and imported `tripChangeProposalLabels`.
- `tests/trip-projects.test.ts` — UPDATED: 5 AC1 tests + 3 patch tests (real existing non-leg parent, backup cross-project/missing/cycle rejection via create/update, stale-aggregate reorder, duplicate-ordinal unique-index cause).
- `_bmad-output/implementation-artifacts/7-6-verify-owner-scoped-trip-planning-safety.md` — UPDATED: story record (baseline_commit, status, task + review-finding checkboxes, Dev Agent Record, Completion Notes incl. concurrency finding, File List, Change Log).
- `_bmad-output/implementation-artifacts/sprint-status.yaml` — UPDATED: story 7.6 status synchronized to review; header comment synchronized.
