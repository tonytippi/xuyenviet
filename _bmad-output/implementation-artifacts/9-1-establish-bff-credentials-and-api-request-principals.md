...
- Second and final bounded independent review 2026-07-28 of exact range `2d9228ed5b8e8d3480c8e23dc231b4d18cd20cff..5c401383e592dcf6b6aeb64dc86ce99adfb18b52` ran Blind Hunter, Edge Case Hunter, and Acceptance Auditor synchronously. BLOCKED: status remains `in-progress`; no code changes were made.
- Actionable findings:
  - PATCH [high][AC2/security] `src/features/admin/actions.ts:200,228`, `src/db/schema.ts:213`, `apps/api/src/auth/resource-server.guard.ts:40-42`: granting or revoking a role never increments `users.authorization_version`. A credential minted before a role revocation retains its stale JWT role snapshot for its remaining lifetime even though the guard is intended to reject authorization-version changes. Increment the target user's version in the same transaction only when the role mutation changes a row, and add a regression that an already-minted credential is rejected after both grant and revoke.
  - PATCH [medium][AC3/test coverage] `tests/api-request-principal.integration.test.ts:106-115`: the rotation/isolation coverage rejects an admin key that claims the web issuer, but never accepts a valid `xuyenviet-admin-bff` token signed with its matching admin key. Add a legitimate admin-issuer acceptance case, retaining the cross-issuer rejection case.
  - PATCH [medium][AC1/test coverage] `tests/bff-credentials.test.ts:128-139`: the non-disclosure assertion serializes a hand-written object rather than an actual BFF route/action response which receives or mints a credential. Exercise a real browser-facing serialization boundary and prove it excludes the credential, session token, and private JWK material.
  - PATCH [medium][configuration hardening] `packages/config/src/index.ts:39-42,84-90`: API configuration coerces a string lifetime such as `"300"` and only superficially checks JWK coordinate strings. Require a finite integer lifetime and validate public/private JWK importability during startup so malformed credentials fail closed before requests.
- Recovery completed (2026-07-28): user-authorized scope decision keeps the authorization-version increment in Story 9.1 as AC2's credential-invalidation security invariant, rather than deferring it to Story 9.2. `grantAdminUserRole` and `revokeAdminUserRole` increment the target user's `authorization_version` in their existing audited transaction only when their respective insert/delete changed a role row; role governance, bootstrap policies, and the retired provisioner were not modified.
