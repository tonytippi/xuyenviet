...
- [x] [Review][Patch] S8.3-R8: Clear executor on source-removal human terminal supersession [`src/features/knowledge/source-removal.ts`] -- Source removal now clears `executorSystem` while recording the resolving operator; the regression starts with a pipeline-attributed recommendation and asserts the terminal row has no system executor.
- [x] [Review][Patch] S8.3-R9: Fence extraction success on the original lease [`src/features/knowledge/extraction-jobs.ts`] -- Success takes a row lock and verifies the original `lockedAt` and `lockedBy` before any terminal side effect, success transition, or success audit; the regression simulates a reclaimed lease with a reused worker ID and asserts it remains running without a success audit.
