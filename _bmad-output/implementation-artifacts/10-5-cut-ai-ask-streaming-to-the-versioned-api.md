...
- 2026-07-29: Final confirmation AC1 repair prevents BFF timeout, EOF, or reader failure from emitting a standalone synthetic terminal before a complete valid `preparing` prefix. Safe terminal recovery is now permitted only for `preparing` followed by zero or more `delta` frames; malformed, incomplete, or out-of-order prefixes close without a synthetic terminal. Raw-byte timeout, EOF, reader-failure, split-fragment, and out-of-order-prefix regressions pass. Synchronous blind, edge-case, and acceptance reviews found no actionable findings. Focused API/BFF/routing verification passed 70 tests; `pnpm typecheck`, `pnpm build`, and `git diff --check` passed. `pnpm lint` had zero errors and five pre-existing warnings.
- 2026-07-29: Final AC1 confirmation repair completed across both adapters. BFF and API recovery now require complete, protocol-valid `preparing` followed by zero or more complete, protocol-valid `delta` frames before they can append one safe terminal. Type-only, malformed, incomplete, oversized, and out-of-order initial frames close without a standalone terminal; completed raw frames remain unchanged. Synchronous blind and acceptance reviews found no actionable AC1 findings; the blind request to forward incomplete fragments was rejected because the established framing contract deliberately drops incomplete NDJSON at close. Serial verification passed 75 API/BFF/routing tests, 169 command/outbox/context tests, and 154 shell/session tests. `pnpm typecheck`, `pnpm build`, and `git diff --check` passed. `pnpm lint` had zero errors and five pre-existing warnings.
...
- [x] [Review][Patch] BFF can begin an NDJSON stream with a synthetic terminal error before `preparing` [src/server/bff-api-client.ts] — repaired by permitting synthetic terminal recovery only after the complete valid prefix `preparing`, zero or more `delta`; timeout, EOF, reader failure, incomplete fragments, and out-of-order records before that prefix close without a standalone terminal error.
- [x] [Review][Patch] BFF prefix gate accepts type-only invalid records [src/server/bff-api-client.ts:260] — repaired by validating complete `preparing` and `delta` frame shapes before advancing recovery eligibility, while relaying complete raw bytes unchanged.
- [x] [Review][Patch] API adapter emits a synthetic terminal after invalid or incomplete stream bytes [apps/api/src/ai-ask/ai-ask.controller.ts:38] — repaired by replacing byte-based `sourceBegun` recovery with the same complete valid `preparing`, optional valid `delta` prefix gate as the BFF.
- [ ] [Review][Patch] Nested `done.assistantMessage` validation accepts an arbitrary third property [apps/api/src/ai-ask/ai-ask.controller.ts:138; src/server/bff-api-client.ts:275] — a malformed `done` record such as `assistantMessage: { id, content, unexpected }` is treated as terminal and suppresses a later valid terminal. Restrict its keys to `id`, `content`, and optional array `provenance` at both adapters.
- [ ] [Review][Patch] Enabled BFF-to-API raw-byte integration fixture asserts obsolete type-only terminal behavior [tests/ai-ask-bff-api.integration.test.ts:80-106] — it expects malformed `{"type":"done"}` to stop relaying, but the repaired validator correctly forwards it and the following record. Replace the fixture with a complete protocol-valid terminal and retain exact byte relay assertions.

### Final-Final Repair Completion

- [x] [Review][Patch] Nested `done.assistantMessage` validation now allows only `id`, `content`, and optional array `provenance` at both raw framers. A malformed nested `done` relays unchanged but cannot terminalize or suppress a later valid terminal.
- [x] [Review][Patch] The enabled BFF-to-API fixture now emits a complete protocol-valid `done` terminal and compares original noncanonical raw UTF-8 bytes across the composed seam.
- 2026-07-29: Synchronous blind, edge-case, and acceptance reviews found no actionable findings after repair. Serial verification passed 79 API/BFF, 169 command/outbox/context, and 154 shell/session tests; `pnpm typecheck`, `pnpm build`, and `git diff --check` passed. `pnpm lint` reported zero errors and five pre-existing warnings. A concurrent combined test launch was rejected because reset-based PostgreSQL suites share one test database; the required serial rerun passed.
