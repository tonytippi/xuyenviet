...
- [ ] Define the versioned streaming contract and extracted API-safe use-case seam (AC: 1-3)
...
- [ ] Add the protected Nest `POST /v1/ai-ask/stream` adapter (AC: 1-2)
...
- [ ] Replace the legacy Next route with a thin authenticated BFF stream relay (AC: 1-3)
...
- [ ] Make routing select exactly one writer and prove cutover safety (AC: 3)
...
- [ ] Prove API, BFF, protocol, persistence, and single-owner behavior (AC: 1-3)
