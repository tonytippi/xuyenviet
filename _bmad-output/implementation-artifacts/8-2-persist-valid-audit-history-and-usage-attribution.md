...
- [x] [Review][Patch] Reject null actor identity fields in database XOR checks [drizzle/migrations/0069_persist_audit_usage_attribution.sql:9] — Migration and Drizzle schema predicates now explicitly require non-null `actor_email` and `actor_system`. Migration-backed tests reject the formerly NULL-bypassable user email and system-ID rows.
- [x] [Review][Patch] Preserve trip-project attribution for web-search usage [src/features/retrieval/source-bundle.ts:146] — Web-search usage context and fallback now carry the available `tripProjectId`; a database-backed source-bundle regression proves the persisted usage row retains project scope.
- [x] [Review][Patch] Keep the normalized usage insert private to Audit [src/features/usage/events.ts:37] — Removed the public normalized writer and its input type from Usage. `writeAiUsageEvent` is the sole public insert boundary and validates the executor catalog before normalization or insertion.
- [x] [Review][Defer] System pipeline audit rows retain legacy fake-user attribution [src/features/knowledge/ingestion-pipeline.ts:234] — deferred to Story 8.3 by this story’s explicit worker/system direct-insert exclusion; pre-existing and not a substantial new risk from this repair range.
- [x] [Review][Defer] Proposal expiry audit row retains legacy fake-user attribution [src/features/chat-trips/trip-change-proposals.ts:2110] — deferred to Story 8.4 by this story’s explicit expiry-wiring exclusion; pre-existing and not a substantial new risk from this repair range.
...
- 2026-07-27 second review repair: synchronized the migration and Drizzle XOR checks to reject SQL NULL bypasses, carried trip-project context through web-search usage, and made Audit the only public normalized usage writer. Focused migration/usage/source-bundle coverage passed (3 files, 106 tests); lint passed with 0 errors and 3 pre-existing unrelated warnings; typecheck and build passed. `pnpm db:generate` remains blocked because Drizzle demands a TTY for rename disambiguation.
