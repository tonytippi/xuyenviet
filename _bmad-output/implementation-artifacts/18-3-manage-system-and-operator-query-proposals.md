...
4. **Given** an authorized operator manages a proposal, **when** they create a simple operator query or manage an existing proposal, **then** every command is role-protected, audited, and preserves `origin = operator | system`.
   - Text edit is available only for an `operator`-origin proposal. A `system`-origin proposal retains its system-derived target/query identity and rejects text edit, while remaining reprioritizable, pausable, and resumable.
...
- [ ] Extend the existing proposal aggregate for safe idempotent planning (AC: 2, 3, 5)
  ...
  - [ ] Story 18.3 has not been deployed. Its unapplied migration is a schema-only clean break: do not transform, backfill, preserve, or schedule any pre-18.3 proposal rows.
...
- [ ] Add protected, audited operator query commands and safe projections (AC: 4, 5)
  - [ ] Add narrow command/read contracts for simple create, operator-origin text edit, priority change, pause, and resume. Reject text edits for system-origin proposals before persistence; retain their system-derived target/query identity. Allow reprioritize, pause, and resume for either origin without allowing command input to select or impersonate a system actor.
...
- [ ] Prove safety, idempotency, scheduling, and command behavior (AC: 1-5)
  ...
  - [ ] Add server-side API tests for unauthenticated/traveler denial, operator/admin admission through `admin.knowledge.write`, immutable origin, real authenticated-user audit attribution, safe output parsing, operator-origin text edit, system-origin text-edit rejection, and reprioritize/pause/resume for both origins.
...
- Automated planning requires the immutable `system-youtube-discovery` actor. Operator commands require the authenticated user actor. Only operator-origin proposals support text edit. A system-origin proposal's text and derived target identity are immutable to operator commands, but it remains reprioritizable, pausable, and resumable. Preserve origin across every command; a system proposal must not become operator-origin or vice versa.
...
- 2026-08-08: Final closure-audit repair only. Operator text edits now match `origin = operator`, preserving system-derived target/query identity. Direct enabled system creation persists a PostgreSQL schedule anchor and the first future `nextDueAt`. The finite Worker poll races owner port reads against the bounded 1-second policy timeout and converts a non-settling read to `source_timeout`; no generic framework was introduced. Safe admin projection parsing now rejects contradictory `enabled`/pause states and accepts only canonical UTC millisecond ISO timestamps. Evidence is explicit in focused planning/foundation/execution tests for competing planning claims, unavailable-outcome persistence, policy revocation immediately before every proposal upsert and due-run insertion, exact singleton scheduling boundaries, no-backfill query pause/resume, system-origin command invariants, and database-level prohibited-data persistence exclusion. Focused direct Vitest unit (2 files, 5 tests) and integration (2 files, 26 tests), complete typecheck, complete build, and whitespace validation pass. Project-wide `pnpm test:unit` and `pnpm test:integration` still execute unrelated suites despite file filters and fail only on pre-existing non-Story-18.3 tests; status remains ready-for-dev.
- 2026-08-08: Final AC closure repair only. Knowledge now publishes bounded coverage, freshness, and unresolved-conflict tuples from its own review/card state; AI Ask publishes thresholded anonymized demand from its own retrieval-decision aggregate. Neither port reads Discovery tables or returns traveler, source, prompt, answer, or provider content. Owner-port integration proves a real Knowledge coverage aggregate idempotently creates one system proposal without persisting private card text. Timed reads receive an `AbortSignal`; a late port resolution is proved unable to write after the timeout. Migration 0047 now disables transformed legacy system operator requests and clears both schedule projections, with migration-backed proof. Focused planning/migration/foundation/execution verification passed (32 tests), as did full typecheck, full build, and diff validation. Status remains ready-for-dev.
- 2026-08-08: Approved Story 18.3 boundary clarified without application-code change: only operator-origin proposals support text edit. System-origin proposals retain system-derived text/target identity but remain reprioritizable, pausable, and resumable. Existing focused integration coverage proves the rejected system edit and all three allowed system controls. Status remains ready-for-dev pending final independent audit.
- 2026-08-08: User confirmed Story 18.3 has never been deployed. Migration 0047 is therefore schema-only and contains no legacy proposal transformation, identity backfill, or schedule projection; the clean-break regression rejects future proposal-row updates in that migration.
- 2026-08-08: Final independent closure audit is clean. Blind Hunter and Edge Case Hunter found no actionable findings; Acceptance Auditor confirmed AC1-AC5 and scope pass. Final repairs standardize policy/proposal lock order, fence paused proposal runs across claim/terminal/retry boundaries, bound scheduler batches, count distinct AI Ask travelers, and fence/order eligible Knowledge recommendations before the 100-signal limit. Focused verification passed: 51 integration tests, 29 unit tests, full typecheck, and diff validation. Status synchronized to done on the uncommitted verified worktree.
