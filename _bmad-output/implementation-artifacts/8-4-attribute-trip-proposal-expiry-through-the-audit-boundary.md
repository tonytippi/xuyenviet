...
- [x] [Review][Patch] Detect chained direct history writers [tests/trip-change-proposals.test.ts:1418] — the source-level enforcement now rejects every `.insert(tripPlanChangeHistory)` form regardless of receiver shape, including `getDb().insert(...)`, with a chained-receiver regression.
- [x] [Review][Defer] Withhold elapsed proposals if best-effort read expiry fails [src/features/chat-trips/trip-change-proposals.ts:930] — deferred, pre-existing. A transient expiry failure can leave an elapsed row in the pending query because it filters only status; the behavior predates baseline `a95aeb7` and is outside this bounded Story 8.4 repair.
