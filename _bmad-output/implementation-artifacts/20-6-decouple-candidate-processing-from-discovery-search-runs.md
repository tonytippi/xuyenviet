...
- [ ] Define candidate-job persistence and migration (AC: 1, 2, 5)
  ...
  - [ ] Add only a policy-owned bounded candidate backlog threshold required for admission. Preserve existing policy values through immutable version creation and validate the threshold at the policy boundary.
- [ ] Make query runs search-and-enqueue only (AC: 1, 3, 4)
  ...
  - [ ] Before claiming/admitting a due query run, enforce the bounded queued/retrying candidate-job backlog. Return only a safe backpressure observation/result and make no YouTube request at or above the threshold.
...
- [ ] Verify migration, isolation, fencing, and safety (AC: 1-5)
  - [ ] Extend DB-free execution tests for stage routing, safe error mapping, backpressure admission, and no-query-retry behavior.
  ...
  - [ ] Extend AI triage/Usage and admin contract/Health tests for immutable provenance, strict safe projections, and no raw/prohibited fields.
