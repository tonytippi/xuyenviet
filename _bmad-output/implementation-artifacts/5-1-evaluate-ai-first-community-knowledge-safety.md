...
- [x] [Review][Patch] Evaluation fixtures permanently publish synthetic active knowledge [src/features/feedback/evaluation-fixtures.ts:42] — Scoped evaluation retrieval permits only the current fixture IDs. Setup keeps fixtures suppressed, and cleanup deletes their full synthetic graph after result persistence so they cannot affect traveler retrieval or future runs.
...
- [x] [Review][Patch] Active evaluation fixtures can be retrieved by travelers [src/features/feedback/evaluation-fixtures.ts:42] — Fixtures are inserted as `suppressed`, are never indexed, and ordinary traveler retrieval remains active-card/index-only. The evaluation path can load only exact `public_mvp_evaluation_fixture_v1` fixture IDs through an explicit internal scope; cleanup deletes the graph after each persisted result. Regression coverage verifies normal retrieval cannot see fixtures while a scenario is running or afterward.
...
- [x] [Review][Patch] Evaluation fixture graphs accumulate indefinitely [src/features/feedback/evaluation-fixtures.ts:90] — After each scenario's result persistence, cleanup transactionally deletes its synthetic card, cascading links/evidence, clears fixture-only source capture pointers, and deletes fixture captures/sources. Durable evaluation result, policy, retrieval-decision, and provenance snapshots remain retained; repeated-run DB coverage proves no synthetic fixture rows enter traveler retrieval or indexing.
...
- 2026-07-24: Resolved the final Epic 5 review finding affecting Story 5.1 only. Evaluation cleanup now deletes the full fixture-owned graph after scenario result persistence while retaining safe persisted result/policy/provenance records. DB-backed repeated-run coverage confirms cards, sources, captures, links, and evidence do not accumulate; traveler retrieval and indexing remain isolated. Serial verification passed: `pnpm test:run tests/public-mvp-evaluation.test.ts` (17), `pnpm typecheck`, and `pnpm build`; `pnpm lint` completed with only the three pre-existing warnings in `tests/knowledge-search.test.ts`. Status moved to `review`; no commit was created.
...
- 2026-07-24: Repaired the final Epic 5 review finding: fixture-owned cards, sources, captures, links, and evidence are deleted after each scenario result persists, while safe evaluation result/policy/retrieval/provenance records remain durable. Story 5.1 moved to `review`; Epic 5 remains `done`; no commit was created.
