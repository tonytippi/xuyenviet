...
- [ ] [Review][Patch] Exposure flags do not inspect the final answer [src/features/feedback/evaluation.ts:524] — `staleWithdrawnSourceExposure` and `rawEvidenceLeakage` only regex-match safe serialized provenance, which is intentionally stripped of raw/withdrawn data. A final answer that repeats withdrawn material or raw evidence is recorded as safe, defeating the AC 2 policy counters.
...
- [ ] [Review][Patch] Failed scenario-contract results discard their policy snapshot [src/features/feedback/evaluation.ts:278] — When a persisted answer-time contract fails, the coordinator creates only a failed result. It does not persist the bounded snapshot already built, preventing durable audit of the unsafe conflict, withdrawal, or fallback outcome that caused the failure.
