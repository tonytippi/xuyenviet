...
- [ ] Replace legacy schema surfaces in `packages/database/src/schema.ts`. (AC: 1-3)
  ...
  - [ ] Jobs: technical `status` only (`queued|running|completed|failed`) and the four defined aggregate counters.
  - [ ] Candidates: `processing_status`, `ai_disposition`, and `outcome_reason_code`.
  - [ ] Recommendations: target work type/status/resolution; target immutable sampling-obligation table.
...
- [ ] Replace legacy contract/domain types and direct inserts in seeds and test helpers. Do not retain aliases or compatibility fixtures. (AC: 1, 4)
- [ ] Reset/reseed only under existing disposable-target safeguards and prove all fixtures satisfy target constraints. (AC: 1, 4)
- [ ] Add database-backed constraint, index, trigger, and target-only fixture tests. (AC: 2-4)
