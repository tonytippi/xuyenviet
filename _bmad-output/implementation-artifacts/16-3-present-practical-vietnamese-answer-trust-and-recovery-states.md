...
- [ ] Make feedback a compact answer-footer interaction without changing its contract (AC: 1, 4)
  ...
  - [ ] Reveal optional targeted reason/comment input only after negative feedback. It must remain optional, keyboard/touch reachable, and must not block composing, move the composer, or consume card-sized vertical space.
  ...
  - [ ] Provide a polite local or global announcement for feedback save success/failure and pending state. All footer controls and the optional save action must meet the 44px target floor and visible-focus convention.
...
- [ ] Preserve detail/sheet focus and reduced-motion behavior (AC: 4-5)
  ...
  - [ ] Add `motion-reduce:transition-none` to Story 16.3 controls that use local `transition`, following existing sheet-close controls. Preserve the global reduced-motion fallback in `apps/web/src/app/globals.css`.

- [ ] Add focused regression coverage (AC: 1-5)
  - [ ] Extend `tests/ai-ask-direct-api.test.ts` for strict `loadAnswerDetail()` and `saveDirectAnswerUsefulnessFeedback()` cookie-authenticated/CSRF request and safe-parser behavior.
  - [ ] Preserve and extend `tests/answer-annotations.test.ts` and `tests/knowledge-source-removal.test.ts` coverage for persisted-provenance-only detail, cross-message/unsafe annotation rejection, withdrawn provenance, and safe unavailable projections.
  - [ ] Add focused component-compatible or source-level web tests using the existing Vitest setup. Cover section chips without mandatory answer cards; absence of banned technical trust/provenance UI; persisted-only compact disclosure; unavailable disclosure; lightweight feedback submission/retry without composer blocking; Vietnamese stream/recovery copy; live-region, 44px, focus-return, and reduced-motion conventions. Do not introduce a new UI-test framework for this story.
