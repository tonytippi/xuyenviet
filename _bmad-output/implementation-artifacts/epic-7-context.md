...
- UI foundations belong in `apps/web/src/app/layout.tsx` and `apps/web/src/app/globals.css`: Inter, Vietnamese document language, semantic white/stone/green/amber/teal/source tokens, base surfaces, focus treatment, and reduced-motion behavior. Reusable presentational primitives are data-free under `apps/web/src/components/ui`; use one local typed SVG icon boundary and no competing feature-local icon systems.
