...
- [x] [Review][Patch] Preserve target-database migration lock interoperability [scripts/migrate-api-schema.ts:13] — the migration command now takes advisory lock `918_040_004` on the application target connection, matching the pre-Epic-12 namespace. The serial PostgreSQL regression holds a legacy target lock and proves the current runner cannot enter; the same lock remains independently acquirable on `/postgres`, proving the test protects the database-scoped interoperability requirement. Disposable reset takes target then maintenance locks before destruction and explicitly relies on its required no-runtime-overlap confirmation after `DROP DATABASE` releases target sessions.
- [ ] [Review][Patch] Retain target migration exclusion after database recreation [scripts/db-reset.ts:145] — `DROP DATABASE` terminates the target lock holder, while reset retains only the maintenance-database lock and then migrates, records the release, and seeds the recreated target. A concurrent `db:migrate` can acquire the target-scoped lock in that interval. Reconnect, identity-verify, and coordinate on the recreated target through reset completion, with a concurrent reset/migrator regression.
...
- 2026-08-01: Final closure migration-lock repair restores `db:migrate` advisory lock `918_040_004` to the target application database, where pre-Epic-12 migration runners hold it. The disposable-reset coordinator documents target-then-maintenance pre-destruction ordering and the required no-runtime-overlap boundary after target destruction. Serial `DATABASE_URL_TEST` migration/lock regressions passed (2 files, 33 tests); typecheck and `git diff --check` passed; lint had zero errors and five existing unrelated warnings. Final synchronous review is clean.
- 2026-08-01: Scope correction approved by the product owner. `db:reset` is a destructive disposable local/test maintenance command, not an overlapping-runtime release path. Its existing disposable-target and explicit no-runtime-overlap preflight remain required, but no cross-recreation runtime-admission fence is in Story 12.3 scope. The post-`DROP DATABASE` runtime-admission finding is dismissed as out-of-scope; deployed runtime and release admission remains governed by the expand-migrate-contract path and Epic 14 evidence.
...
- Closure scope corrected: no durable reset admission protocol was added because `db:reset` remains restricted to disposable local/test maintenance targets and is outside the deployed overlapping-runtime release protocol.
