...
- [ ] Add serial PostgreSQL race and rollback evidence at the established production seams. Each rejection must snapshot and assert card, supporting evidence eligibility, candidate outcome, work, Audit event, dirty marker, search projection, sampling obligation/cohort membership as applicable, then prove they remain unchanged together. Assign cases to the named suites below instead of building a second test harness. (AC: 2)
