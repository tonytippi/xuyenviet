...
| OP-07 Capture boundary and scheduled actor | Repository implementation updated 2026-07-26; deployment evidence pending | 2026-07-26 | `drizzle/migrations/0065_system_facebook_capture_actor.sql`; `scripts/facebook-capture.ts`; `tests/facebook-capture-script.test.ts`; `docs/runbooks/facebook-capture.md` | Every capture run is bound to the fixed reserved `system-facebook-capture` actor; the CLI accepts no per-run actor override and records `actorClass=system`/`actorSystem=system-facebook-capture` in capture audit events. Interactive mode requires confirmation before save; `--yes` only removes that confirmation. No controlled deployed run was supplied. | Amelia | `blocked` | Repository identity-spoofing defect is repaired, but deployment migration/identity verification and a controlled capture exercise are absent. | Apply migration; run an interactive and a `--yes` fixture capture proving fixed system attribution, then retain safe audit/status evidence. |
...
- OP-07's original repository identity-spoofing defect is repaired by the fixed system-actor implementation. It remains blocked pending deployment migration and controlled capture exercises.
