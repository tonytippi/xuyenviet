...
- [x] [Review][Patch] Upgrade preflight makes the clean-break contract explicit [drizzle/migrations/0072_reject_system_executor_user_ids.sql; tests/story-8-6-actor-isolation.test.ts] — **High repaired.** Before adding its reserved-ID check, migration 0072 detects historic catalog user IDs and stops with an explicit reset/reseed requirement. It does not delete dependent data, backfill, or claim durable-upgrade support. The isolated upgrade-state regression proves the preflight fails before installing the constraint, then proves a cleaned disposable state can install it and rejects a reserved ID.
- [x] [Review][Patch] Computed namespace-table aliases are rejected by the Audit direct-write guard [tests/story-8-6-actor-isolation.test.ts] — **Medium repaired.** String-literal namespace `ElementAccessExpression` values now resolve as protected tables; regressions reject both `db.insert(schema["auditEvents"])` and an alias created from that expression.
- [x] [Review][Patch] Computed schema keys respect lexical shadowing [tests/story-8-6-actor-isolation.test.ts] — **Medium repaired.** Computed namespace keys resolve their nearest preceding lexical `const` string binding at the reference, rather than retaining inner-block bindings in mutable analysis state. The regression verifies an outer `"auditEvents"` key remains protected after an inner `"other"` shadow ends.
...
- 2026-07-27: Second bounded BMad code review through `84ae1a5d4122bfe3687d4404e058bf9e1c4abb36` ran Blind Hunter, Edge Case Hunter, and Acceptance Auditor. Two unresolved patch findings remain: High substantial new risk that migration 0072 cannot upgrade databases retaining historic catalog user rows, and Medium computed namespace-table alias bypass in the Audit direct-write guard. Status set to `in-progress`. No implementation, test, migration, database, or commit action was performed.
