...
- [x] Remove fake seed identities while preserving deliberate people and human provenance (AC: 1, 3)
   - [x] In `scripts/db-seed.ts`, remove the `system-facebook-capture`, `system-youtube-capture`, and traveler user fixtures. Retain only `seed-fixture-operator-user` for valid Facebook/YouTube source-submitter provenance.
...
- [x] Prove the clean migration and seed result (AC: 1, 3)
   - [x] Add focused Story 8.5 Vitest coverage using the existing `tests/helpers/db.ts`/`DATABASE_URL_TEST` conventions for repeatable database regressions: prove the four reserved IDs are absent from `users`, no user ID starts with `system-`, the operator fixture preserves source-submitter provenance, the traveler fixture graph is absent, and a cataloged system audit/executor write does not require a matching user row. Do not point a test at `DATABASE_URL`, invoke `pnpm db:reset` from Vitest, or treat a truncated test database as a clean migration/seed proof.
  - [x] Prove a valid cataloged system audit/executor write succeeds without a matching `users` row. Reuse `tests/audit-actors.test.ts` and `tests/audit-attribution-migration.test.ts` for generic actor XOR behavior rather than duplicating their matrix.
   - [x] If an authoritative development reset is explicitly authorized, run `pnpm db:reset` against the preflighted `DATABASE_URL`. Query that same reset database for IDs/emails matching historic system identities and every `users.id LIKE 'system-%'`; expected result: zero rows. Confirm only `seed-fixture-operator-user` remains and its Facebook/YouTube source-submitter relationships are preserved. Do not substitute a successful `DATABASE_URL_TEST` run for this check.
...
- `scripts/db-seed.ts` retains only the real operator fixture for Facebook/YouTube source submitter provenance. The traveler fixture and its trip/chat graph are intentionally absent.
...
- Historical verification note: the prior reset result with a traveler role, trip, and conversation is superseded by the 2026-07-27 single-operator seed contract. Future verification must assert only operator source-submitter provenance and traveler-graph absence.
