...
**Always:** Treat Story 21.1 in `story-contracts.md` as exact. Keep the payload to bounded intent, flat scoped slots, missing-slot names, status, source message IDs, and revision. Reuse existing conversation, message, retrieval-decision, Trip, Usage, provenance, feedback, and audit storage. Finalize exactly `drizzle/migrations/0073_clean_break_trip_aware_planning.sql` with only `planning_context_sessions` and preserve serial integration testing.
...
- `drizzle/migrations/0072_discovery_activate_vietnamese_query_builder.sql` -- current final migration before the required `0073` clean-break migration.
...
- `packages/database/src/schema.ts` and `drizzle/migrations/0073_clean_break_trip_aware_planning.sql` -- add only the owner-linked `planning_context_sessions` table with needed cascade, index, and constraints -- finalize Epic 21 schema without unrelated cleanup.
...
**Acceptance Criteria:**
- Given the approved `DATABASE_URL_TEST` target passes its existing safety validation, when migration `0073_clean_break_trip_aware_planning.sql` runs, then only `planning_context_sessions` is added and existing conversation, message, retrieval-decision, Trip, Usage, provenance, feedback, and audit tables are reused without unrelated cleanup.
