...
export function MissionDetail({ actionId }: { actionId: string }) {
  ...
  const more = useRef<HTMLButtonElement>(null); const heading = useRef<HTMLHeadingElement>(null); const [status, setStatus] = useState("Đang tải dấu vết an toàn.");
    ...
    try {
      ...
      if (!response.ok || !parsed || id !== request.current) throw new Error("unavailable");
       setDetail((current) => append && current ? { ...parsed, candidates: { items: [...current.candidates.items, ...parsed.candidates.items], nextCursor: parsed.candidates.nextCursor } } : parsed);
        setUnavailable(false); setStatus(append ? "Đã tải thêm ứng viên xếp hạng." : "Đã tải dấu vết nhu cầu an toàn."); if (!append) requestAnimationFrame(() => heading.current?.focus());
       if (append) requestAnimationFrame(() => more.current?.focus());
    } catch (error) { if ((error as Error).name !== "AbortError" && id === request.current) { setUnavailable(true); setStatus("Không thể tải dấu vết nhu cầu lúc này."); } } finally { if (id === request.current) setLoadingMore(false); }
  }, [actionId]);
