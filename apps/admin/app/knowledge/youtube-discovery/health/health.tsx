...
export function YoutubeDiscoveryHealthQuality({ quality }: { quality: AdminYoutubeDiscoveryHealthOverview["quality"] }) { const thresholdMet = quality.vietnameseFitPercent !== null && quality.vietnameseFitPercent >= 80; return <section className="mt-6 border border-[#b8c4b9] bg-[#fbf7ed] p-5"><h2 className="font-semibold">Chất lượng ưu tiên tiếng Việt</h2><p className="mt-2 text-sm">Tỷ lệ “Nên xem xét” phù hợp tiếng Việt: {quality.vietnameseFitPercent === null ? "Chưa có mẫu" : `${quality.vietnameseFitPercent}%`}. Mẫu: {quality.vietnameseConsider}/{quality.considered}. Ngưỡng yêu cầu: 80% ({quality.vietnameseFitPercent === null ? "chưa đánh giá" : thresholdMet ? "đạt" : "chưa đạt"}).</p><p className="mt-2 text-sm text-slate-600">Nguồn ngoại ngữ bổ sung: {quality.foreignFallback}. Quá ngắn: {quality.tooShort}; chưa rõ thời lượng: {quality.durationUnknown}; không phải tiếng Việt: {quality.nonVietnamese}; chưa rõ ngôn ngữ: {quality.languageUnknown}; vi phạm thời lượng: {quality.durationViolations}.</p></section>; }
function Incidents({ incidents }: { incidents: AdminYoutubeDiscoveryHealthOverview["incidents"] }) { return <section className="mt-6 border border-[#b8c4b9] bg-[#fbf7ed] p-5"><h2 className="font-semibold">Sự cố cần xử lý</h2>{incidents.length === 0 ? <p className="mt-3 text-sm">Không có sự cố cần xử lý.</p> : <ol className="mt-3 divide-y divide-[#b8c4b9] border-y border-[#b8c4b9]">{incidents.map((incident) => { const reason = incident.reason as keyof typeof incidentCopy; return <li key={incident.actionId}><Link className="flex min-h-11 items-center justify-between gap-4 px-3 py-2 font-semibold underline decoration-2 underline-offset-4" href={`/knowledge/youtube-discovery/health/${encodeURIComponent(incident.actionId)}`} aria-label={`Mở sự cố: ${incidentCopy[reason]}`}><span>{incidentCopy[reason]}</span><span className="text-right text-sm font-normal">Ưu tiên {incident.priority}. Ghi nhận: {format(incident.occurredAt)}.</span></Link></li>; })}</ol>}</section>; }
