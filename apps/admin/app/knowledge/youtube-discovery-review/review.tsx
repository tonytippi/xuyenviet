...
import { parseAdminYoutubeDiscoveryAcceptReviewResult, parseAdminYoutubeDiscoveryBrowsePage, parseAdminYoutubeDiscoveryDeferReviewResult, parseAdminYoutubeDiscoveryReviewDetail, parseAdminYoutubeDiscoveryReviewQueue, parseAdminYoutubeDiscoverySkipReviewResult, type AdminYoutubeDiscoveryBrowseFilter, type AdminYoutubeDiscoveryBrowseItem, type AdminYoutubeDiscoveryReviewDetail, type AdminYoutubeDiscoveryReviewQueueItem } from "@xuyenviet/contracts";
...
export function YoutubeDiscoveryReview() {
  ...
  return <main className="mx-auto max-w-7xl p-4 text-slate-900 sm:p-8"><header><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-sm font-semibold text-emerald-800">YOUTUBE DISCOVERY</p><h1 ref={heading} tabIndex={-1} className="mt-2 text-3xl font-bold outline-none">Youtube Discovery</h1><p className="mt-3 text-slate-600">Duyệt từng URL ứng viên. Xếp hạng không xác nhận nội dung, thu thập hay xuất bản.</p></div><Link className="inline-flex min-h-11 items-center rounded border border-emerald-900 px-4 font-semibold text-emerald-900" href="/knowledge/youtube-discovery/health">Health Discovery</Link></div></header><p aria-live="polite" className="mt-4" role="status">{status}</p><section aria-labelledby="review-queue" className="mt-6 min-w-0"><h2 id="review-queue" ref={queueHeading} tabIndex={-1} className="text-xl font-bold outline-none">Hàng đợi</h2><div aria-label="Lọc hàng đợi" className="mt-3 flex flex-wrap gap-2" role="tablist">{tabs.map(([filter, label]) => <button aria-controls="review-queue-content" aria-selected={browseFilter === filter} className="min-h-11 rounded border px-4 aria-selected:border-emerald-800 aria-selected:bg-emerald-50" id={`review-tab-${filter}`} key={filter} onClick={() => setBrowseFilter(filter)} role="tab" type="button">{label}</button>)}</div><div aria-labelledby={`review-tab-${browseFilter}`} id="review-queue-content" role="tabpanel">{isConsiderTab ? considerSurface : browseSurface}</div></section></main>;
}
