...
export function projectUserRoleCommand(page: AdminUserRosterPage, command: UserRoleCommandResult): AdminUserRosterPage {
  ...
  return {
    ...
    items: page.items.map((user) => {
      if (user.id !== command.targetUserId) return user;
      const roles = user.roles.filter((item) => item !== command.role);
      if (command.operation === "grant") roles.push(command.role);
      return { ...user, roles: roles.sort() };
    }),
  };
}

export function parseExpectedUserRoleCommand(value: unknown, expected: Omit<UserRoleCommandResult, "changed">): UserRoleCommandResult | null {
  const command = parseUserRoleCommandResult(value);
