...
| Safe evidence reference | Redacted log/query/export location, approved release-matrix path, or ticket URL |
...
| `GATE-MIGRATION-WRITER` | `BLOCKED` | Database operator | Before dependent traffic, execute the approved forward-only migration procedure and verify the selected writer against the approved release matrix and owner inventory. | The deployed schema revision, migration-before-traffic order, one selected writer, `dualWrite: false`, and matrix approval all match retained evidence. | Matrix, target identity, migration order, selected writer, or one-writer evidence is missing; a down migration or dual write is proposed. |
...
| `GATE-ROLLBACK-DRILL` | `BLOCKED` | Incident commander | Execute the approved compatible rollback drill without a down migration or destructive cleanup; retain the measured recovery, approved matrix-selected `legacyOwnerId`, and selected legacy binary revision. | The drill switches traffic to the matrix-selected legacy writer only, keeps `dualWrite: false`, proves the replaced writer accepts no commands, and restores the compatible service path within its approved threshold while preserving forward-only schema/data constraints. | The drill is unexecuted, exceeds its threshold, requires a down migration, has no safe retained result, leaves more than one writer active, or uses an unapproved fallback. |
| `GATE-API-WORKER-READINESS` | `BLOCKED` | API owner and Worker operator | Verify deployed API health/version/protected capability and Worker live/ready, schema compatibility, drain, and replacement readiness. | API and Worker readiness are healthy on the deployed revision; Worker loops are ready and the controlled drain/restart outcome is retained. | Any readiness endpoint, schema compatibility check, Worker loop, drain, or recovery proof is missing or failing. |
...
1. Select the staging target, accountable release operator, approved release matrix, and immutable deployed revision. If any is unavailable, stop with every staging gate `BLOCKED`.
...
5. Select the production target and deployed revision. Reconfirm the release matrix, migration-before-traffic plan, and selected writer before production traffic, then execute `GATE-MIGRATION-WRITER` before every dependent production gate.
...
Use `docs/runbooks/schema-release-matrix.md` for the forward-only migration and selected-writer procedure, `docs/release-matrices/README.md` for release-matrix admissibility, and `docs/runbooks/worker-operations.md` for Worker readiness, drain, telemetry, and alert controls.
