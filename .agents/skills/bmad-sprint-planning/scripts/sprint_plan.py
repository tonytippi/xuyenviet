...
def _compact_story_key(key):
    """Return a punctuation-insensitive identity for historical story keys."""
    m = STORY_KEY_RE.match(key)
    if not m:
        return None
...
def reconcile_story_keys(entries, existing_data):
    """Retain a unique historical spelling for the same numbered story key."""
    if existing_data is None:
        return entries
    existing_status = dict(existing_data.get("development_status") or {})
    existing_by_identity = {}
    for key in existing_status:
        identity = _compact_story_key(str(key))
        if identity is not None:
            existing_by_identity.setdefault(identity, []).append(str(key))

    reconciled = []
    for key, kind, epic_num in entries:
        identity = _compact_story_key(key) if kind == "story" else None
        candidates = existing_by_identity.get(identity, []) if identity is not None else []
        if key not in existing_status and len(candidates) == 1:
            key = candidates[0]
        reconciled.append((key, kind, epic_num))
    return reconciled
...
def cmd_generate(args):
    ...
    entries = reconcile_story_keys(entries, existing)
