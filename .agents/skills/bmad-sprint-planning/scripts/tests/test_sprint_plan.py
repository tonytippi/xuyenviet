...
def test_long_story_titles_preserve_existing_status_keys(tmp_path, capsys):
    title = "Create Immutable Source Capture Versions And Retention Boundaries"
    key = "3-2-create-immutable-source-capture-versions-and-retention-boundaries"
    epics = f"## Epic 3: Knowledge\n### Story 3.2: {title}\n"
    existing = f"""\
generated: 01-01-2026 09:00
last_updated: 01-01-2026 09:00
project: My Project
...
"""
    ...
    assert load(status_file)["development_status"][key] == "done"
    assert result["new_entries"] == []
    assert result["dropped_orphans"] == []


def test_historical_story_key_spelling_is_reconciled(tmp_path, capsys):
    epics = "## Epic 7: Trips\n### Story 7.3: Present Owner's Workspace\n"
    existing = """\
generated: 01-01-2026 09:00
last_updated: 01-01-2026 09:00
project: My Project
development_status:
  epic-7: done
  7-3-present-owners-workspace: done
...
"""
    ...
    assert load(status_file)["development_status"]["7-3-present-owners-workspace"] == "done"
    assert result["new_entries"] == []
    assert result["dropped_orphans"] == []
