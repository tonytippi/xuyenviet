...
type CheckpointCandidate = { type: (typeof knowledgeCardTypeValues)[number]; title: string; summary: string; locationName: string | null; routeSegment: string | null; conditions: string[]; freshnessSensitive: boolean; spanStart: number; spanEnd: number; modelId: string; promptVersion: string };
...
export async function commitKnowledgeIngestionStage(input: KnowledgeIngestionStageCommit, db: IngestionJobDb = getDb()) {
  ...
  if (input.checkpoint && !parseCheckpoint(input.checkpoint)) throw new KnowledgeIngestionJobError("Checkpoint is invalid.");
  const [committed] = await db.update(knowledgeIngestionJobs).set({ stage: input.nextStage, stageVersion: input.expectedStageVersion + 1, checkpoint: terminal ? null : input.checkpoint, lastErrorCode: input.lastErrorCode ?? null, ...(terminal ? { claimedBy: null, claimedAt: null, leaseExpiresAt: null, fencingToken: null } : {}), updatedAt: now }).where(and(eq(knowledgeIngestionJobs.id, input.jobId), eq(knowledgeIngestionJobs.stage, input.expectedStage), eq(knowledgeIngestionJobs.stageVersion, input.expectedStageVersion), eq(knowledgeIngestionJobs.fencingToken, input.fencingToken), gt(knowledgeIngestionJobs.leaseExpiresAt, now))).returning();
...
function parseCheckpointCandidate(value: unknown): CheckpointCandidate | null { if (!isRecord(value) || !knowledgeCardTypeValues.includes(value.type as CheckpointCandidate["type"]) || !bounded(value.title, 160) || !bounded(value.summary, 1200) || (value.locationName !== null && !bounded(value.locationName, 160)) || (value.routeSegment !== null && !bounded(value.routeSegment, 160)) || !Array.isArray(value.conditions) || value.conditions.length > 12 || value.conditions.some((item) => !bounded(item, 160)) || typeof value.freshnessSensitive !== "boolean" || !Number.isInteger(value.spanStart) || !Number.isInteger(value.spanEnd) || !bounded(value.modelId, 160) || !bounded(value.promptVersion, 160)) return null; const spanStart = value.spanStart as number; const spanEnd = value.spanEnd as number; if (spanStart < 0 || spanEnd <= spanStart) return null; return { type: value.type as CheckpointCandidate["type"], title: value.title as string, summary: value.summary as string, locationName: value.locationName as string | null, routeSegment: value.routeSegment as string | null, conditions: value.conditions as string[], freshnessSensitive: value.freshnessSensitive, spanStart, spanEnd, modelId: value.modelId as string, promptVersion: value.promptVersion as string }; }
function parseCheckpointJudgment(value: unknown): CheckpointJudgment | null { if (!isRecord(value) || !["publish", "review_recommended", "verify_first", "suppress"].includes(String(value.decision)) || !bounded(value.summary, 1000)) return null; const keys = ["relevance", "extractability", "evidenceGrounding", "specificity", "actionability", "firstHandLikelihood", "spamCommercialRisk"] as const; if (keys.some((key) => typeof value[key] !== "number" || value[key] < 0 || value[key] > 1)) return null; return { decision: value.decision as CheckpointJudgment["decision"], summary: value.summary as string, relevance: value.relevance as number, extractability: value.extractability as number, evidenceGrounding: value.evidenceGrounding as number, specificity: value.specificity as number, actionability: value.actionability as number, firstHandLikelihood: value.firstHandLikelihood as number, spamCommercialRisk: value.spamCommercialRisk as number }; }
function parseCheckpointRelation(value: unknown): CheckpointRelation | null { if (!isRecord(value) || !["attach", "create", "conflict"].includes(String(value.action)) || !bounded(value.summary, 1000) || (value.targetCardId !== null && !bounded(value.targetCardId, 160)) || (["attach", "conflict"].includes(String(value.action)) && !value.targetCardId)) return null; return { action: value.action as CheckpointRelation["action"], targetCardId: value.targetCardId as string | null, summary: value.summary as string }; }
