...
function normalizeConstraints(input: InternalConstraintsInput) {
  ...
  if (Object.keys(input).some((key) => !allowed.has(key))) throw new Error("Invalid trip constraints fields.");
  const adultCount = input.adultCount ?? null; const childCount = input.childCount ?? null;
  ...
  if (input.evChargingNeed && input.vehicleType !== "ev") throw new Error("Invalid trip constraints EV need.");
  if (input.drivingToleranceHours !== null && input.drivingToleranceHours !== undefined && (!Number.isInteger(input.drivingToleranceHours) || input.drivingToleranceHours < 1 || input.drivingToleranceHours > 12)) throw new Error("Invalid trip constraints driving tolerance.");
  if ((input.budgetMinVnd ?? null) !== null || (input.budgetMaxVnd ?? null) !== null || input.budgetCurrency) if (input.budgetCurrency !== "VND" || !Number.isInteger(input.budgetMinVnd) || !Number.isInteger(input.budgetMaxVnd) || input.budgetMinVnd! < 0 || input.budgetMaxVnd! < input.budgetMinVnd! || input.budgetMaxVnd! > 1_000_000_000) throw new Error("Invalid trip constraints budget.");
  ...
  if (input.children && (!Array.isArray(input.children) || input.children.length > 10 || input.children.some((child) => !isChildConstraint(child, childComfortTags, childPreferenceTags)))) throw new Error("Invalid trip constraints children.");
  if (input.preferenceTags && (!Array.isArray(input.preferenceTags) || input.preferenceTags.length > 20 || new Set(input.preferenceTags).size !== input.preferenceTags.length || input.preferenceTags.some((tag) => !preferenceTags.has(tag)))) throw new Error("Invalid trip constraints preferences.");
  if (input.avoidItems && (!Array.isArray(input.avoidItems) || input.avoidItems.length > 20 || input.avoidItems.some((item) => !isAvoidItem(item)))) throw new Error("Invalid trip constraints avoid items.");
  return { adultCount, childCount, children: input.children ?? null, vehicleType: input.vehicleType ?? null, evChargingNeed: input.evChargingNeed ?? null, drivingToleranceHours: input.drivingToleranceHours ?? null, budgetCurrency: input.budgetCurrency ?? null, budgetMinVnd: input.budgetMinVnd ?? null, budgetMaxVnd: input.budgetMaxVnd ?? null, preferenceTags: input.preferenceTags ?? null, avoidItems: input.avoidItems ?? null };
}
