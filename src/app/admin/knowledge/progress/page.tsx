...
import { knowledgeCardTypeLabels, recommendationReasonLabels, sourceIntakeActionLabels } from "@/features/knowledge/display-labels";
...
export default async function KnowledgeProgressPage() {
  ...
  return (
    <div>
      <p className="text-sm font-semibold uppercase tracking-[0.2em] text-[#8c4f13]">Mức độ bao phủ tri thức có bằng chứng</p>
      ...
      <p className="mt-5 max-w-2xl text-lg leading-8 text-[#4f625a]">
        Chỉ đếm thẻ hiện hành có thông tin truy xuất đầy đủ, đoạn bằng chứng còn được lưu và nguồn vẫn đủ điều kiện. Phê duyệt trước đây không đồng nghĩa với việc đã sẵn sàng. Màn hình chỉ hiển thị số liệu tổng hợp an toàn.
      </p>
      ...
      <section className="mt-8 rounded-[1.5rem] border border-[#d8c9ad] bg-white/75 p-5 shadow-[0_12px_30px_rgba(41,33,18,0.08)] sm:p-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          ...
          <p className="rounded-full border border-[#d8c9ad] bg-[#fbf7ed] px-4 py-2 text-sm font-semibold text-[#4f625a]">
            {progress.isComplete ? "Đã đạt mục tiêu hiện hành" : `Còn thiếu ${progress.remainingActiveCards} thẻ`}
          </p>
        </div>
        <div className="mt-5 h-4 overflow-hidden rounded-full bg-[#e2d3ba]" aria-label={`Đã đạt ${percent}% mục tiêu`}>
      ...
      {closedSamplingPolicies.some((policy) => !policy.enrollmentSealedAt) ? (
        <section className="mt-8 rounded-[1.5rem] border border-[#d8c9ad] bg-white/70 p-5 sm:p-6">
          <h2 className="text-2xl font-semibold tracking-[-0.03em] text-[#17342c]">Niêm phong đợt lấy mẫu đã kết thúc</h2>
          <p className="mt-2 leading-7 text-[#4f625a]">Thao tác này chỉ kiểm tra nhật ký và danh sách thành viên không thể thay đổi, rồi lưu bằng chứng xác nhận cho đợt đã kết thúc; không thay đổi thẻ hoặc khuyến nghị.</p>
          <div className="mt-4 grid gap-3">
            {closedSamplingPolicies.filter((policy) => !policy.enrollmentSealedAt).map((policy) => (
              <form action={sealClosedKnowledgeSamplingPolicyForm} className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-[#e2d3ba] bg-[#fbf7ed] p-4" key={policy.id}>
                ...
                <button className="min-h-11 rounded-xl bg-[#1f5f46] px-4 font-semibold text-white" type="submit">Niêm phong đợt lấy mẫu</button>
              </form>
            ))}
          </div>
        </section>
      ...
      <dl className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <ProgressSignal label="Cần kiểm tra" count={progress.pendingReviewCards} />
      ...
      <section className="mt-8 rounded-[1.5rem] border border-[#d8c9ad] bg-white/70 p-5 sm:p-6">
        ...
        <p className="mt-2 leading-7 text-[#4f625a]">Gồm các khuyến nghị đang mở hoặc xử lý, có phiên bản khớp với thẻ hiện tại, và các nguồn đủ điều kiện liên quan đến hành lang hiện hành.</p>
        {progress.actionableWork.length === 0 ? <p className="mt-4 text-[#4f625a]">Chưa có công việc hiện hành cho hành lang này.</p> : (
          <ul className="mt-4 grid gap-3">
            {progress.actionableWork.map((item) => <li className="flex items-center justify-between gap-4 rounded-2xl border border-[#e2d3ba] bg-[#fbf7ed] p-4" key={`${item.kind}:${item.priority ?? "intake"}:${item.reason}`}><span className="font-semibold text-[#17342c]">{item.priority === null ? "Nạp nguồn" : `Ưu tiên ${item.priority}`} · {(item.kind === "source_intake" ? sourceIntakeActionLabels[item.reason] : recommendationReasonLabels[item.reason]) ?? item.reason}</span><span className="rounded-full bg-white/80 px-3 py-1 text-sm font-semibold text-[#4f625a]">{item.count}</span></li>)}
          </ul>
        )}
      ...
      <section className="mt-8 grid gap-6 lg:grid-cols-2">
        <ProgressList title="Phân bổ theo loại thẻ" empty="Chưa có thẻ đủ điều kiện trong hành lang này." items={progress.byType.map((item) => ({ label: knowledgeCardTypeLabels[item.type] ?? item.type, count: item.count }))} />
        <ProgressList title="Phân bổ theo tuyến đường hoặc địa điểm" empty="Chưa có tuyến đường hoặc địa điểm đủ điều kiện trong hành lang này." items={progress.byRouteOrLocation.map((item) => ({ label: item.routeOrLocation, count: item.count }))} />
      </section>
    </div>
  );
}
