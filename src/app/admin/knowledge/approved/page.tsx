...
export default async function ApprovedKnowledgePage({ searchParams }: ApprovedKnowledgePageProps) {
  ...
  return (
    <div>
      ...
      <h1 className="mt-4 max-w-3xl text-4xl font-semibold tracking-[-0.04em] sm:text-5xl">Tri thức đã xuất bản.</h1>
      ...
      <form className="mt-8 rounded-[1.5rem] border border-[#d8c9ad] bg-white/75 p-4 shadow-[0_12px_30px_rgba(41,33,18,0.08)] sm:p-5" action="/admin/knowledge/approved">
        <label className="text-sm font-semibold uppercase tracking-[0.18em] text-[#8c4f13]" htmlFor="approved-knowledge-search">
          Tìm trong chỉ mục tri thức đã xuất bản
        </label>
        ...
        <p className="mt-3 text-sm leading-6 text-[#4f625a]">Kết quả dùng chung chỉ mục với Trợ lý AI; tiến trình nền chịu trách nhiệm lập chỉ mục và cập nhật.</p>
      </form>

      {params.approved ? (
        <p className="mt-6 rounded-2xl border border-[#8fb59f] bg-[#edf7ef] px-4 py-3 font-semibold text-[#1f5f46]" role="status">
          Thẻ {params.approved} đã được phê duyệt. Kiểm tra nguồn, độ tin cậy và độ mới tại danh sách này.
        </p>
      ) : null}
        ...
        {searchMode ? (
          <>
            <div className="rounded-[1.5rem] border border-[#d8c9ad] bg-[#fbf7ed] p-5">
              <h2 className="text-2xl font-semibold tracking-[-0.03em] text-[#17342c]">Kết quả tìm kiếm: {query}</h2>
              <p className="mt-3 leading-7 text-[#4f625a]">
                Tìm thấy {searchResult?.candidateCount ?? 0} thẻ đã lập chỉ mục, hiển thị {searchResult?.results.length ?? 0} kết quả phù hợp nhất.
              </p>
            </div>
            {searchResult?.results.length === 0 ? (
              <div className="rounded-[1.5rem] border border-[#d8c9ad] bg-white/70 p-5">
                <h2 className="text-2xl font-semibold tracking-[-0.03em] text-[#17342c]">Không tìm thấy trong chỉ mục</h2>
                <p className="mt-3 leading-7 text-[#4f625a]">Thử từ khóa khác hoặc kiểm tra tiến trình lập chỉ mục nếu thẻ mong đợi chưa xuất hiện.</p>
              </div>
            ) : (
          ...
          <div className="rounded-[1.5rem] border border-[#d8c9ad] bg-white/70 p-5">
              <h2 className="text-2xl font-semibold tracking-[-0.03em] text-[#17342c]">Chưa có thẻ đã xuất bản</h2>
  ...
  return (
    <article className="rounded-[1.5rem] border border-[#d8c9ad] bg-white/75 p-5 shadow-[0_12px_30px_rgba(41,33,18,0.08)]">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <div className="mt-3 flex flex-wrap gap-2 text-sm font-semibold">
            ...
            {typeof score === "number" ? <span className="rounded-full border border-[#d8c9ad] bg-[#fbf7ed] px-3 py-1 text-[#4f625a]">Độ phù hợp: {score}</span> : null}
          </div>
        </div>
      ...
      <dl className="mt-5 grid gap-3 text-sm sm:grid-cols-2">
        ...
        {card.indexStatus ? (
          <div className="rounded-2xl bg-[#fbf7ed] p-3">
            <dt className="font-semibold text-[#17342c]">Chỉ mục / điều kiện sử dụng</dt>
