...
export default async function KnowledgeRecommendationPage({ params, searchParams }: Props) {
  ...
  return <div>
    ...
    {recommendation.candidate ? <section className="mt-7 rounded-2xl border border-[#d8c9ad] bg-[#f4ead7] p-5">
      ...
      <p className="mt-2 text-sm leading-6 text-[#4f625a]">Đây là bản ghi pipeline đã tạo thẻ này. Dùng để đối chiếu với thẻ tri thức trước khi quyết định xử lý.</p>
      <dl className="mt-4 grid gap-3 text-sm sm:grid-cols-2">
        <div className="rounded-xl bg-white/70 p-3"><dt className="font-semibold">Trạng thái pipeline</dt><dd className="mt-1 text-[#4f625a]">{candidateStageLabels[recommendation.candidate.stage] ?? recommendation.candidate.stage}</dd></div>
      ...
      {recommendation.candidate.tags.length > 0 ? <p className="mt-3 text-sm text-[#4f625a]"><span className="font-semibold text-[#17342c]">Nhãn candidate: </span>{recommendation.candidate.tags.join(" · ")}</p> : null}
    </section> : null}
    <section className="mt-7 rounded-2xl border border-[#d8c9ad] bg-white/75 p-5">
      ...
      <h3 className="mt-5 text-lg font-semibold">Fact và bằng chứng</h3>
