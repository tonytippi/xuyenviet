      ...
      {recommendations.length === 0 ? <p className="rounded-2xl border border-[#d8c9ad] bg-white/70 p-5 text-[#4f625a]">Không có khuyến nghị phù hợp.</p> : recommendations.map((item) => <article className="rounded-2xl border border-[#d8c9ad] bg-white/75 p-5" key={item.id}>
        <div className="flex flex-col justify-between gap-4 sm:flex-row"><div>
          ...
          {workStatus === "actionable" && item.reason === "verification" ? <div className="flex flex-wrap gap-2">
            <form action={promoteKnowledgeRecommendationFromQueueForm}><QueueActionInputs item={item} page={page} reason={reason} /><button className="min-h-11 rounded-xl bg-[#8c4f13] px-4 py-3 font-semibold text-white" type="submit">Xuất bản dạng thông tin đã xác minh</button></form>
            <form action={verifyKnowledgeRecommendationFromQueueForm}><QueueActionInputs item={item} page={page} reason={reason} /><button className="min-h-11 rounded-xl bg-[#1f5f46] px-4 py-3 font-semibold text-white" type="submit">Xuất bản dạng cảnh báo</button></form>
