...
function QueueActionInputs({ item, page, reason }: { item: { id: string; contentVersion: number; evidenceSetRevision: number }; page: number; reason?: KnowledgeRecommendationReason }) {
  return <>
    <input name="recommendationId" type="hidden" value={item.id} />
    <input name="contentVersion" type="hidden" value={item.contentVersion} />
    <input name="evidenceSetRevision" type="hidden" value={item.evidenceSetRevision} />
...
export default async function KnowledgeRecommendationsPage({ searchParams }: Props) {
  ...
  return <div>
    ...
    <section aria-label="Tổng quan hàng đợi" className="mt-7 grid gap-3 sm:grid-cols-3">
      {knowledgeRecommendationWorkStatusValues.map((value) => <Link className={`rounded-2xl border p-4 transition focus:outline-none focus:ring-4 ${workStatus === value ? "border-[#1f5f46] bg-[#edf7ef] focus:ring-[#8fb59f]" : "border-[#d8c9ad] bg-white/75 hover:border-[#8c4f13] focus:ring-[#d8c9ad]"}`} href={`/admin/knowledge/recommendations?${new URLSearchParams({ workStatus: value, ...(reason ? { reason } : {}) })}`} key={value}>
        <p className="text-sm font-semibold text-[#4f625a]">{workStatusLabels[value]}</p><p className="mt-2 text-3xl font-semibold text-[#17342c]">{workStatusCounts[value]}</p><p className="mt-1 text-sm text-[#4f625a]">{value === "actionable" ? "Đang chờ quyết định" : value === "completed" ? "Đã có kết quả xử lý" : "Không cần thao tác thêm"}</p>
      </Link>)}
    </section>
    ...
    {params.error ? <p className="mt-5 rounded-xl bg-[#f4ead7] p-4 text-sm text-[#17342c]">Không thể hoàn tất thao tác. Mục này không đủ điều kiện hiện hành để đưa vào coverage.</p> : null}
    <section className="mt-7 grid gap-4">
      {recommendations.length === 0 ? <p className="rounded-2xl border border-[#d8c9ad] bg-white/70 p-5 text-[#4f625a]">Không có khuyến nghị phù hợp.</p> : recommendations.map((item) => <article className="rounded-2xl border border-[#d8c9ad] bg-white/75 p-5" key={item.id}>
        <div className="flex flex-col justify-between gap-4 sm:flex-row"><div>
          <p className="text-sm font-semibold text-[#8c4f13]">{workStatusLabels[workStatus]} · {recommendationReasonLabels[item.reason] ?? item.reason}{workStatus === "actionable" ? ` · Ưu tiên ${item.priority}` : ""}</p>
          <h2 className="mt-2 text-xl font-semibold text-[#17342c]">{item.card.title}</h2><p className="mt-2 text-[#4f625a]">{item.card.summary}</p>
          <p className="mt-3 text-sm text-[#4f625a]">{workStatus === "completed" ? resolutionLabels[item.resolution ?? ""] ?? "Đã hoàn tất xử lý" : workStatus === "inactive" ? "Công việc này không còn đang hoạt động." : "Mở để đối chiếu bằng chứng và đưa ra quyết định."}</p>
        </div><div className="flex h-fit flex-col gap-2">
          {workStatus === "actionable" && item.reason === "verification" ? <div className="flex flex-wrap gap-2">
            <form action={verifyKnowledgeRecommendationFromQueueForm}><QueueActionInputs item={item} page={page} reason={reason} /><button className="min-h-11 rounded-xl bg-[#8c4f13] px-4 py-3 font-semibold text-white" type="submit">Xuất bản dạng thông tin đã xác minh</button></form>
            <form action={promoteKnowledgeRecommendationFromQueueForm}><QueueActionInputs item={item} page={page} reason={reason} /><button className="min-h-11 rounded-xl bg-[#1f5f46] px-4 py-3 font-semibold text-white" type="submit">Xuất bản thành ghi nhận cộng đồng</button></form>
            <form action={suppressKnowledgeRecommendationFromQueueForm}><QueueActionInputs item={item} page={page} reason={reason} /><button className="min-h-11 rounded-xl border border-[#8c4f13] px-4 py-3 font-semibold text-[#8c4f13]" type="submit">Không xuất bản</button></form>
            <Link className="inline-flex min-h-11 items-center justify-center rounded-xl border border-[#d8c9ad] px-4 py-3 font-semibold text-[#17342c]" href={`/admin/knowledge/recommendations/${encodeURIComponent(item.id)}`}>Xem chi tiết</Link>
          </div> : <Link className="inline-flex min-h-11 items-center justify-center rounded-xl border border-[#d8c9ad] px-4 py-3 font-semibold text-[#17342c]" href={`/admin/knowledge/recommendations/${encodeURIComponent(item.id)}`}>Xem chi tiết</Link>}
        </div></div>
      </article>)}
    </section>
