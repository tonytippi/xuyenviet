...
export const youtubeDiscoveryRunSafeErrorCodeValues = ["stage_transient", "search_transient", "search_timeout", "enrichment_transient", "execution_timeout", "triage_transient", "triage_timeout", "eligibility_unavailable", "recommendation_transient", "persistence_contended", "retry_exhausted", "lease_retry_exhausted", "policy_revoked"] as const;
...
export const youtubeDiscoveryPolicyVersions = pgTable(
  "youtube_discovery_policy_versions",
  {
    ...
    languageClassifierVersion: integer("language_classifier_version").notNull(),
    minimumUsefulDurationSeconds: integer("minimum_useful_duration_seconds").notNull(),
    allowForeignFallback: boolean("allow_foreign_fallback").notNull(),
  ...
  (policy) => [
    ...
    check("youtube_discovery_policy_versions_language_classifier_version_check", sql`${policy.languageClassifierVersion} between 0 and 1 and ${policy.minimumUsefulDurationSeconds} between 180 and 86400`),
...
export const youtubeDiscoveryQueryProposals = pgTable(
  "youtube_discovery_query_proposals",
  {
    ...
    operatorPriorityOverride: integer("operator_priority_override"),
  ...
  (proposal) => [
    ...
    check("youtube_discovery_query_proposals_operator_priority_override_check", sql`${proposal.operatorPriorityOverride} is null or (${proposal.origin} = 'system' and ${proposal.operatorPriorityOverride} between 1 and 100)`),
...
export const youtubeDiscoveryRuns = pgTable(
  "youtube_discovery_runs",
  {
    ...
    immediateOperatorUserId: text("immediate_operator_user_id"),
    immediateConfirmationKey: text("immediate_confirmation_key"),
  ...
  (run) => [
    ...
    uniqueIndex("youtube_discovery_runs_immediate_confirmation_idx").on(run.queryProposalId, run.immediateOperatorUserId, run.immediateConfirmationKey).where(sql`${run.queryProposalId} is not null and ${run.immediateOperatorUserId} is not null and ${run.immediateConfirmationKey} is not null`),
    check("youtube_discovery_runs_immediate_confirmation_check", sql`(${run.immediateOperatorUserId} is null and ${run.immediateConfirmationKey} is null) or (${run.immediateOperatorUserId} is not null and ${run.immediateConfirmationKey} is not null and ${run.queryProposalId} is not null and ${run.scheduleIntervalAt} is null and length(btrim(${run.immediateOperatorUserId})) between 1 and 128 and ${run.immediateConfirmationKey} ~ '^[A-Za-z0-9_-]{16,128}$')`),
    ...
    check("youtube_discovery_runs_error_code_check", sql`${run.safeErrorCode} is null or ${run.safeErrorCode} in ('stage_transient', 'search_transient', 'search_timeout', 'enrichment_transient', 'execution_timeout', 'triage_transient', 'triage_timeout', 'eligibility_unavailable', 'recommendation_transient', 'persistence_contended', 'retry_exhausted', 'lease_retry_exhausted', 'policy_revoked')`),
    ...
    check("youtube_discovery_runs_state_shape_check", sql`(${run.state} = 'queued' and ${run.claimedBy} is null and ${run.terminalAt} is null and ${run.terminalOutcome} is null and ${run.safeErrorCode} is null) or (${run.state} = 'running' and ${run.claimedBy} is not null and ${run.terminalAt} is null and ${run.terminalOutcome} is null and ${run.safeErrorCode} is null) or (${run.state} = 'retrying' and ${run.claimedBy} is null and ${run.nextRunAt} > ${run.createdAt} and ${run.terminalAt} is null and ${run.terminalOutcome} is null and ${run.safeErrorCode} in ('stage_transient', 'search_transient', 'search_timeout', 'enrichment_transient', 'execution_timeout', 'triage_transient', 'triage_timeout', 'eligibility_unavailable', 'recommendation_transient', 'persistence_contended')) or (${run.state} in ('completed', 'failed', 'cancelled') and ${run.claimedBy} is null and ${run.terminalAt} is not null and ${run.terminalOutcome} = ${run.state} and (${run.state} <> 'failed' or ${run.safeErrorCode} in ('stage_transient', 'search_transient', 'search_timeout', 'enrichment_transient', 'execution_timeout', 'triage_transient', 'triage_timeout', 'eligibility_unavailable', 'recommendation_transient', 'persistence_contended', 'retry_exhausted', 'lease_retry_exhausted')) and (${run.state} <> 'cancelled' or ${run.safeErrorCode} = 'policy_revoked') and (${run.state} <> 'completed' or ${run.safeErrorCode} is null))`),
  ],
);
...
export const youtubeDiscoveryAppearances = pgTable(
  "youtube_discovery_appearances",
   { id: text("id").primaryKey().$defaultFn(() => crypto.randomUUID()), candidateId: text("candidate_id").notNull().references(() => youtubeDiscoveryCandidates.id, { onDelete: "restrict" }), runId: text("run_id").notNull().references(() => youtubeDiscoveryRuns.id, { onDelete: "restrict" }), resultOrdinal: integer("result_ordinal").notNull(), searchTranche: text("search_tranche").$type<YoutubeDiscoverySearchTranche>(), title: text("title"), description: text("description"), channelId: text("channel_id"), channelName: text("channel_name"), publishedAt: timestamp("published_at", { mode: "date" }), durationSeconds: integer("duration_seconds"), defaultLanguage: text("default_language"), defaultAudioLanguage: text("default_audio_language"), languageFit: text("language_fit").$type<"vi" | "likely_vi" | "unknown" | "non_vi">(), durationFit: text("duration_fit").$type<"eligible" | "too_short" | "duration_unknown">(), eligibilityReason: text("eligibility_reason").$type<"eligible_vietnamese" | "too_short" | "duration_unknown" | "non_vietnamese" | "language_unknown" | "foreign_fallback">(), queryBuilderVersion: integer("query_builder_version"), languageClassifierVersion: integer("language_classifier_version"), minimumUsefulDurationSeconds: integer("minimum_useful_duration_seconds"), categoryId: text("category_id"), tags: text("tags").array(), viewCount: integer("view_count"), likeCount: integer("like_count"), commentCount: integer("comment_count"), channelSubscriberCount: integer("channel_subscriber_count"), thumbnailUrl: text("thumbnail_url"), discoveredAt: timestamp("discovered_at", { mode: "date" }).defaultNow().notNull() },
    (appearance) => [uniqueIndex("youtube_discovery_appearances_run_candidate_idx").on(appearance.runId, appearance.candidateId), uniqueIndex("youtube_discovery_appearances_id_candidate_run_idx").on(appearance.id, appearance.candidateId, appearance.runId), index("youtube_discovery_appearances_candidate_idx").on(appearance.candidateId), check("youtube_discovery_appearances_ordinal_check", sql`${appearance.resultOrdinal} between 0 and 49`), check("youtube_discovery_appearances_search_tranche_check", sql`${appearance.searchTranche} is null or ${appearance.searchTranche} in ('medium', 'long')`), check("youtube_discovery_appearances_eligibility_check", sql`(${appearance.languageFit} is null and ${appearance.durationFit} is null and ${appearance.eligibilityReason} is null and ${appearance.queryBuilderVersion} is null and ${appearance.languageClassifierVersion} is null and ${appearance.minimumUsefulDurationSeconds} is null) or (${appearance.languageFit} in ('vi','likely_vi','unknown','non_vi') and ${appearance.durationFit} in ('eligible','too_short','duration_unknown') and ${appearance.eligibilityReason} = case when ${appearance.durationFit} = 'too_short' then 'too_short' when ${appearance.durationFit} = 'duration_unknown' then 'duration_unknown' when ${appearance.languageFit} in ('vi','likely_vi') then 'eligible_vietnamese' when ${appearance.eligibilityReason} = 'foreign_fallback' and ${appearance.languageFit} in ('unknown','non_vi') then 'foreign_fallback' when ${appearance.languageFit} = 'non_vi' then 'non_vietnamese' else 'language_unknown' end and ${appearance.queryBuilderVersion} between 1 and 2 and ${appearance.languageClassifierVersion} = 1 and ${appearance.minimumUsefulDurationSeconds} between 180 and 86400)`)],
);
...
export const youtubeDiscoveryTriages = pgTable("youtube_discovery_triages", {
  ...
  foreignKey({ columns: [triage.appearanceId, triage.candidateId, triage.runId], foreignColumns: [youtubeDiscoveryAppearances.id, youtubeDiscoveryAppearances.candidateId, youtubeDiscoveryAppearances.runId], name: "youtube_discovery_triages_appearance_provenance_fk" }).onDelete("restrict"),
  foreignKey({ columns: [triage.runId, triage.policyVersionId], foreignColumns: [youtubeDiscoveryRuns.id, youtubeDiscoveryRuns.policyVersionId], name: "youtube_discovery_triages_run_policy_fk" }).onDelete("restrict"),
  ...
  check("youtube_discovery_triages_signals_check", sql`${triage.signals} is null or (${triage.signals} <@ array['recent_discussion','stale_or_changed_warning','practical_question_demand','creator_responsiveness','commercial_risk','contradictory_discussion']::text[] and cardinality(${triage.signals}) between 0 and 6 and coalesce(cardinality(array_positions(${triage.signals}, 'recent_discussion')), 0) <= 1 and coalesce(cardinality(array_positions(${triage.signals}, 'stale_or_changed_warning')), 0) <= 1 and coalesce(cardinality(array_positions(${triage.signals}, 'practical_question_demand')), 0) <= 1 and coalesce(cardinality(array_positions(${triage.signals}, 'creator_responsiveness')), 0) <= 1 and coalesce(cardinality(array_positions(${triage.signals}, 'commercial_risk')), 0) <= 1 and coalesce(cardinality(array_positions(${triage.signals}, 'contradictory_discussion')), 0) <= 1)`),
