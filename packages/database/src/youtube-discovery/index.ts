...
async function transitionYoutubeDiscoveryCandidateJob(claim: YoutubeDiscoveryCandidateJobClaim, requested: "completed" | "cancelled", database: DiscoveryWriter): Promise<YoutubeDiscoveryCandidateJobDisposition> {
  return database.transaction(async (transaction) => {
    ...
    if (updated) await recordCandidateJobTerminalAudit(transaction, updated.id, outcome, claim.attemptCount, outcome === "cancelled" ? "policy_revoked" : null, claim.policyVersionId);
    return updated ? outcome : "contended";
  });
}
...
function assertSafeYoutubeDiscoveryEnrichment(enrichment: YoutubeDiscoveryEnrichment) {
  const safeText = (value: string | undefined, maximum: number) => value === undefined || value.length <= maximum && !/[\x00-\x1F\x7F]/.test(value);
  if (!safeText(enrichment.title, 200) || !safeText(enrichment.description, 1000) || !safeText(enrichment.channelName, 160) || enrichment.tags?.some((tag) => tag.length > 80 || /[\x00-\x1F\x7F]/.test(tag))) throw new Error("YouTube Discovery enrichment metadata is invalid.");
}
...
async function guardYoutubeDiscoveryCandidateWrite(transaction: DiscoveryWriter, claim: YoutubeDiscoveryCandidateWorkClaim): Promise<"contended" | "cancelled" | { readonly policyVersionId: string; readonly queryProposalId: string | null; readonly active: true }> {
  if (isYoutubeDiscoveryCandidateJobClaim(claim)) {
    ...
    if (!job) return "contended";
    const [policy] = await transaction.execute(sql`select enabled from youtube_discovery_policy_versions where is_current = true for update`) as Array<{ enabled: boolean }>;
