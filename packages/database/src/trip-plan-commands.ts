...
import { canonicalRoutePathMatchesEndpoints, isCanonicalRoutePathId } from "./route-coverage";
...
export async function setInternalTripPlanItemCanonicalRoutePathInTransaction(transaction: TripPlanCommandTransaction, actor: TripPlanCommandActor, tripProjectId: string, expectedAggregateVersion: number, itemId: string, expectedItemVersion: number, canonicalRoutePathId: string | null): Promise<AggregateMutationResult> {
  ...
  const project = await lockAggregate(transaction, tripProjectId, actor.userId, expectedAggregateVersion); if (!project.success) return project;
  const [item] = await transaction.select({ version: tripPlanItems.version, type: tripPlanItems.type, transportOriginLabel: tripPlanItems.transportOriginLabel, transportDestinationLabel: tripPlanItems.transportDestinationLabel }).from(tripPlanItems).where(and(eq(tripPlanItems.id, itemId), eq(tripPlanItems.tripProjectId, tripProjectId), eq(tripPlanItems.userId, actor.userId))).limit(1);
  ...
  if (canonicalRoutePathId !== null && !canonicalRoutePathMatchesEndpoints(canonicalRoutePathId, item.transportOriginLabel, item.transportDestinationLabel)) return { success: false, reason: "invalid" };
