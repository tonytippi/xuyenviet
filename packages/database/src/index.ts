...
import { adminUserRosterPageSize, conversationSummaryLimit, encodeAdminUserRosterCursor, parsePlanningAnswerDetailResponse, planningDetailProvenanceLimit, type AdminAiGatewayModel, type AdminAiGatewayModelInput, type AdminAiGatewayModelUpdate, type AdminAiPurposeAssignment, type PlanningJsonValue, type PlanningProvenance, type RequestPrincipal, type RequestRole, type TravelerShellProjection, type TripAnswerContextResponse } from "@xuyenviet/contracts";
...
export * from "./youtube-discovery-mission-action-frontier";
...
export function createPostgresTravelerShellRepository(): TravelerShellRepository {
  return {
    async loadOwnedTravelerShell(userId, conversationId, tripProjectId) {
      ...
      return {
        conversation: conversation ? { ...conversation, messages: ownedMessages.map(({ id, role, content }) => ({ id, role, content: content ?? "" })) } : null,
  ...
  return {
    async list() {
      const [models, assignments] = await Promise.all([
        sql<ModelRow[]>`select id, gateway_model_name as "gatewayModelName", display_label as "displayLabel", active, supports_text_input as "supportsTextInput", supports_image_input as "supportsImageInput", supports_image_output as "supportsImageOutput", supports_embeddings as "supportsEmbeddings", supports_extraction as "supportsExtraction", supports_evaluation as "supportsEvaluation", supports_streaming as "supportsStreaming", supports_cache_pricing as "supportsCachePricing", pricing_currency as "pricingCurrency", input_token_price_micros as "inputTokenPriceMicros", output_token_price_micros as "outputTokenPriceMicros", cache_read_token_price_micros as "cacheReadTokenPriceMicros", cache_write_token_price_micros as "cacheWriteTokenPriceMicros", pricing_unit_tokens as "pricingUnitTokens", pricing_version as "pricingVersion", pricing_effective_at as "pricingEffectiveAt" from ai_gateway_models order by active desc, display_label asc, gateway_model_name asc`,
        sql<AdminAiPurposeAssignment[]>`select purpose, ai_gateway_model_id as "aiGatewayModelId" from ai_purposes order by purpose asc`,
      ]);
      return { models: models.map(serializeModel), assignments };
    },
    async create(principal, input) { return sql.begin(async (transaction) => {
      const actor = await exactAdminActor(transaction, principal);
      const rows = await transaction<ModelRow[]>`insert into ai_gateway_models (id, gateway_model_name, display_label, active, supports_text_input, supports_image_input, supports_image_output, supports_embeddings, supports_extraction, supports_evaluation, supports_streaming, supports_cache_pricing, pricing_currency, input_token_price_micros, output_token_price_micros, cache_read_token_price_micros, cache_write_token_price_micros, pricing_unit_tokens, pricing_version, pricing_effective_at) values (${randomUUID()}, ${input.gatewayModelName}, ${input.displayLabel}, ${input.active}, ${input.supportsTextInput}, ${input.supportsImageInput}, ${input.supportsImageOutput}, ${input.supportsEmbeddings}, ${input.supportsExtraction}, ${input.supportsEvaluation}, ${input.supportsStreaming}, ${input.supportsCachePricing}, ${input.pricingCurrency}, ${input.inputTokenPriceMicros}, ${input.outputTokenPriceMicros}, ${input.cacheReadTokenPriceMicros}, ${input.cacheWriteTokenPriceMicros}, ${input.pricingUnitTokens}, ${input.pricingVersion}, ${new Date(input.pricingEffectiveAt)}) returning id, gateway_model_name as "gatewayModelName", display_label as "displayLabel", active, supports_text_input as "supportsTextInput", supports_image_input as "supportsImageInput", supports_image_output as "supportsImageOutput", supports_embeddings as "supportsEmbeddings", supports_extraction as "supportsExtraction", supports_evaluation as "supportsEvaluation", supports_streaming as "supportsStreaming", supports_cache_pricing as "supportsCachePricing", pricing_currency as "pricingCurrency", input_token_price_micros as "inputTokenPriceMicros", output_token_price_micros as "outputTokenPriceMicros", cache_read_token_price_micros as "cacheReadTokenPriceMicros", cache_write_token_price_micros as "cacheWriteTokenPriceMicros", pricing_unit_tokens as "pricingUnitTokens", pricing_version as "pricingVersion", pricing_effective_at as "pricingEffectiveAt"`;
    ...
    async update(principal, id, input) { return sql.begin(async (transaction) => {
      ...
      const assignments = await transaction<AdminAiPurposeAssignment[]>`select purpose, ai_gateway_model_id as "aiGatewayModelId" from ai_purposes where ai_gateway_model_id = ${id} for update`;
      assignments.forEach((assignment) => validatePurposeModel(assignment.purpose, next));
      const rows = await transaction<ModelRow[]>`update ai_gateway_models set gateway_model_name = ${next.gatewayModelName}, display_label = ${next.displayLabel}, active = ${next.active}, supports_text_input = ${next.supportsTextInput}, supports_image_input = ${next.supportsImageInput}, supports_image_output = ${next.supportsImageOutput}, supports_embeddings = ${next.supportsEmbeddings}, supports_extraction = ${next.supportsExtraction}, supports_evaluation = ${next.supportsEvaluation}, supports_streaming = ${next.supportsStreaming}, supports_cache_pricing = ${next.supportsCachePricing}, pricing_currency = ${next.pricingCurrency}, input_token_price_micros = ${next.inputTokenPriceMicros}, output_token_price_micros = ${next.outputTokenPriceMicros}, cache_read_token_price_micros = ${next.cacheReadTokenPriceMicros}, cache_write_token_price_micros = ${next.cacheWriteTokenPriceMicros}, pricing_unit_tokens = ${next.pricingUnitTokens}, pricing_version = ${next.pricingVersion}, pricing_effective_at = ${new Date(next.pricingEffectiveAt)}, updated_at = now() where id = ${id} returning id, gateway_model_name as "gatewayModelName", display_label as "displayLabel", active, supports_text_input as "supportsTextInput", supports_image_input as "supportsImageInput", supports_image_output as "supportsImageOutput", supports_embeddings as "supportsEmbeddings", supports_extraction as "supportsExtraction", supports_evaluation as "supportsEvaluation", supports_streaming as "supportsStreaming", supports_cache_pricing as "supportsCachePricing", pricing_currency as "pricingCurrency", input_token_price_micros as "inputTokenPriceMicros", output_token_price_micros as "outputTokenPriceMicros", cache_read_token_price_micros as "cacheReadTokenPriceMicros", cache_write_token_price_micros as "cacheWriteTokenPriceMicros", pricing_unit_tokens as "pricingUnitTokens", pricing_version as "pricingVersion", pricing_effective_at as "pricingEffectiveAt"`;
      await catalogAudit(transaction, actor, "update", id, input); return serializeModel(rows[0]!);
    }); },
    async assignPurpose(principal, input) { return sql.begin(async (transaction) => { const actor = await exactAdminActor(transaction, principal); const model = await loadModelForUpdate(transaction, input.aiGatewayModelId); validatePurposeModel(input.purpose, serializeModel(model)); const previous = await transaction<AdminAiPurposeAssignment[]>`select purpose, ai_gateway_model_id as "aiGatewayModelId" from ai_purposes where purpose = ${input.purpose} for update`; await transaction`insert into ai_purposes (purpose, ai_gateway_model_id, updated_at) values (${input.purpose}, ${input.aiGatewayModelId}, now()) on conflict (purpose) do update set ai_gateway_model_id = excluded.ai_gateway_model_id, updated_at = now()`; await purposeAssignmentAudit(transaction, actor, previous[0] ?? null, input); return input; }); },
    async archive(principal, id) { return sql.begin(async (transaction) => { const actor = await exactAdminActor(transaction, principal); await loadModelForUpdate(transaction, id); const mapped = await transaction`select 1 from ai_purposes where ai_gateway_model_id = ${id} for update`; if (mapped.length) throw new AdminAiModelCatalogPolicyError("AI Gateway model is still assigned to a purpose."); const rows = await transaction<ModelRow[]>`update ai_gateway_models set active = false, updated_at = now() where id = ${id} returning id, gateway_model_name as "gatewayModelName", display_label as "displayLabel", active, supports_text_input as "supportsTextInput", supports_image_input as "supportsImageInput", supports_image_output as "supportsImageOutput", supports_embeddings as "supportsEmbeddings", supports_extraction as "supportsExtraction", supports_evaluation as "supportsEvaluation", supports_streaming as "supportsStreaming", supports_cache_pricing as "supportsCachePricing", pricing_currency as "pricingCurrency", input_token_price_micros as "inputTokenPriceMicros", output_token_price_micros as "outputTokenPriceMicros", cache_read_token_price_micros as "cacheReadTokenPriceMicros", cache_write_token_price_micros as "cacheWriteTokenPriceMicros", pricing_unit_tokens as "pricingUnitTokens", pricing_version as "pricingVersion", pricing_effective_at as "pricingEffectiveAt"`; await catalogAudit(transaction, actor, "archive", id, {}); return serializeModel(rows[0]!); }); },
  };
}
...
async function loadModelForUpdate(transaction: postgres.TransactionSql, id: string) { const rows = await transaction<ModelRow[]>`select id, gateway_model_name as "gatewayModelName", display_label as "displayLabel", active, supports_text_input as "supportsTextInput", supports_image_input as "supportsImageInput", supports_image_output as "supportsImageOutput", supports_embeddings as "supportsEmbeddings", supports_extraction as "supportsExtraction", supports_evaluation as "supportsEvaluation", supports_streaming as "supportsStreaming", supports_cache_pricing as "supportsCachePricing", pricing_currency as "pricingCurrency", input_token_price_micros as "inputTokenPriceMicros", output_token_price_micros as "outputTokenPriceMicros", cache_read_token_price_micros as "cacheReadTokenPriceMicros", cache_write_token_price_micros as "cacheWriteTokenPriceMicros", pricing_unit_tokens as "pricingUnitTokens", pricing_version as "pricingVersion", pricing_effective_at as "pricingEffectiveAt" from ai_gateway_models where id = ${id} for update`; if (!rows[0]) throw new AdminAiModelCatalogPolicyError("AI Gateway model not found."); return rows[0]; }
function validateCompleteModel(model: AdminAiGatewayModel) { const priced = [model.inputTokenPriceMicros, model.outputTokenPriceMicros, model.cacheReadTokenPriceMicros, model.cacheWriteTokenPriceMicros].some((value) => value !== null); if (priced && !model.pricingCurrency) throw new AdminAiModelCatalogPolicyError("Pricing currency is required when any token price is configured."); }
function validatePurposeModel(purpose: AdminAiPurposeAssignment["purpose"], model: AdminAiGatewayModel) { if (!model.active) throw new AdminAiModelCatalogPolicyError("Assigned AI Gateway model must be active."); if (purpose === "ai_ask_initial_answer" && !model.supportsTextInput || (purpose === "extraction" || purpose === "youtube_discovery_triage" || purpose === "youtube_discovery_province_suggestion") && (!model.supportsTextInput || !model.supportsExtraction) || purpose === "embeddings" && !model.supportsEmbeddings || purpose === "evaluation" && (!model.supportsTextInput || !model.supportsEvaluation)) throw new AdminAiModelCatalogPolicyError("AI Gateway model does not support this purpose."); }
...
function purposeAssignmentAudit(transaction: postgres.TransactionSql, actor: { id: string; email: string | null }, previous: AdminAiPurposeAssignment | null, next: AdminAiPurposeAssignment) { return transaction`insert into audit_events (id, actor_user_id, actor_email, actor_class, operation, target_type, target_id, before_summary, after_summary) values (${randomUUID()}, ${actor.id}, ${actor.email!}, 'user', 'update', 'ai_purpose', ${next.purpose}, ${previous ? JSON.stringify(previous) : null}, ${JSON.stringify(next)})`; }
