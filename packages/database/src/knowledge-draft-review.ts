...
export async function listApprovedKnowledgeCards(): Promise<ApprovedKnowledgeCard[]> {
  const rows = await getDb()
    ...
    .leftJoin(knowledgeCardSources, eq(knowledgeCardSources.knowledgeCardId, knowledgeCards.id))
    .leftJoin(sources, eq(sources.id, knowledgeCardSources.sourceId))
    .where(and(eq(knowledgeCards.status, "approved"), eq(knowledgeCards.publicationState, "active"), eq(knowledgeCards.needsReview, false)))
...
export async function getApprovedKnowledgeCard(cardId: string): Promise<ApprovedKnowledgeCard | null> {
  ...
  const rows = await getDb()
    ...
    .leftJoin(knowledgeCardSources, eq(knowledgeCardSources.knowledgeCardId, knowledgeCards.id))
    .leftJoin(sources, eq(sources.id, knowledgeCardSources.sourceId))
    .where(and(eq(knowledgeCards.id, normalizedCardId), eq(knowledgeCards.status, "approved"), eq(knowledgeCards.publicationState, "active"), eq(knowledgeCards.needsReview, false)));

  const card = groupApprovedRows(rows)[0];
  return card ?? null;
}

export async function updateKnowledgeDraft(draftId: string, input: KnowledgeDraftUpdateInput, actor: KnowledgeReviewActor): Promise<KnowledgeDraftReviewResult> {
  const normalizedDraftId = draftId.trim();
...
export async function rejectKnowledgeDraft(draftId: string, actor: KnowledgeReviewActor): Promise<KnowledgeDraftReviewResult> {
  const normalizedDraftId = draftId.trim();
...
export async function approveKnowledgeDraft(draftId: string, actor: KnowledgeReviewActor, expectedUpdatedAt?: string | null): Promise<KnowledgeDraftReviewResult> {
  const normalizedDraftId = draftId.trim();
  ...
  const db = getDb();
  return db.transaction(async (transaction) => {
    return approveKnowledgeDraftInTransaction(transaction, actor, normalizedDraftId, expectedUpdatedAt);
  });
}

export async function approveKnowledgeDraftBatch(draftIds: string[], actor: KnowledgeReviewActor): Promise<{ draftIds: string[] }> {
  ...
  const db = getDb();
  return db.transaction(async (transaction) => {
    return approveKnowledgeDraftBatchInTransaction(transaction, actor, normalizedDraftIds);
  });
}

export async function approveKnowledgeDraftBatchInTransaction(transaction: ReviewMutationDb, actor: KnowledgeReviewActor, normalizedDraftIds: string[]): Promise<{ draftIds: string[] }> {
  if (normalizedDraftIds.length === 0) {
    throw new KnowledgeDraftReviewError("Không có bản nháp nào để phê duyệt.", "invalid_draft");
  }
  ...
  for (const draftId of normalizedDraftIds) {
    const result = await approveKnowledgeDraftInTransaction(transaction, actor, draftId);
...
async function approveKnowledgeDraftInTransaction(
  ...
  actor: KnowledgeReviewActor,
  normalizedDraftId: string,
  expectedUpdatedAt?: string | null,
): Promise<KnowledgeDraftReviewResult> {
  const sourceSnapshot = await loadReviewableDraft(transaction, normalizedDraftId);
