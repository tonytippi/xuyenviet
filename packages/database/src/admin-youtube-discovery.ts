...
export function createPostgresAdminYoutubeDiscoveryPort(captureEligibility: YoutubeCaptureEligibilityPort = { async check() { return "unavailable"; } }, db: AdminYoutubeDiscoveryDatabase = getDb(), handoff: AdminYoutubeDiscoveryDependencies["knowledgeHandoff"] = { async submit() { return "reconciling"; }, async lookup() { return "reconciling"; } }, actionOwners: NonNullable<AdminYoutubeDiscoveryDependencies["actionOwners"]> = { async admitsActionCursor() { return false; }, async listMissionNeeds() { return []; }, async listKnowledgeRecommendations() { return []; } }, missionOwners: NonNullable<AdminYoutubeDiscoveryDependencies["missionOwners"]> = { async listMissionCoverage() { return { items: [], nextCursor: null }; }, async getMissionDetail() { return null; } }): AdminYoutubeDiscoveryPort {
  return {
    ...
    async listMissionQueries(cursor) {
      if (cursor) {
        const [anchor] = await db.select({ id: youtubeDiscoveryQueryProposals.id }).from(youtubeDiscoveryQueryProposals).where(and(eq(youtubeDiscoveryQueryProposals.id, cursor.id), eq(youtubeDiscoveryQueryProposals.priority, cursor.priority), eq(youtubeDiscoveryQueryProposals.createdAt, new Date(cursor.createdAt)))).limit(1);
        if (!anchor) throw new YoutubeDiscoveryMissionCursorValidationError("Invalid YouTube Discovery Mission cursor.");
      }
      const rows = await db.select({ id: youtubeDiscoveryQueryProposals.id, origin: youtubeDiscoveryQueryProposals.origin, queryText: youtubeDiscoveryQueryProposals.queryText, reason: youtubeDiscoveryQueryProposals.reason, priority: youtubeDiscoveryQueryProposals.priority, enabled: youtubeDiscoveryQueryProposals.enabled, cadenceMinutes: youtubeDiscoveryQueryProposals.cadenceMinutes, nextDueAt: youtubeDiscoveryQueryProposals.nextDueAt, createdAt: youtubeDiscoveryQueryProposals.createdAt, policyEnabled: youtubeDiscoveryPolicyVersions.enabled }).from(youtubeDiscoveryQueryProposals).leftJoin(youtubeDiscoveryPolicyVersions, eq(youtubeDiscoveryPolicyVersions.isCurrent, true)).where(cursor ? or(gt(youtubeDiscoveryQueryProposals.priority, cursor.priority), and(eq(youtubeDiscoveryQueryProposals.priority, cursor.priority), gt(youtubeDiscoveryQueryProposals.createdAt, new Date(cursor.createdAt))), and(eq(youtubeDiscoveryQueryProposals.priority, cursor.priority), eq(youtubeDiscoveryQueryProposals.createdAt, new Date(cursor.createdAt)), gt(youtubeDiscoveryQueryProposals.id, cursor.id))) : undefined).orderBy(asc(youtubeDiscoveryQueryProposals.priority), asc(youtubeDiscoveryQueryProposals.createdAt), asc(youtubeDiscoveryQueryProposals.id)).limit(adminYoutubeDiscoveryMissionPageSize + 1);
      ...
      return { items, nextCursor: rows.length > adminYoutubeDiscoveryMissionPageSize && last ? encodeAdminYoutubeDiscoveryMissionQueryCursor({ version: 1, priority: last.priority, createdAt: last.createdAt.toISOString(), id: last.id }) : null };
    },
    ...
    async missionFunnel() {
      ...
      for (const candidate of candidates) { counts[candidate.rankingState] += 1; if (candidate.candidateState !== "unavailable") counts[`${candidate.candidateState === "pending" ? "pendingReview" : candidate.candidateState}`] += 1; }
