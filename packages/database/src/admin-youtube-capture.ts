...
type Row = { sourceId: string; sourceLabel: string; sourceUrl: string | null; sourceCanonicalUrl: string | null; createdAt: Date; rawText: string | null; captureMethod: string | null; capturedAt: string | null; jobId: string | null; jobStatus: "queued" | "running" | "completed" | "failed" | null; jobUpdatedAt: Date | null; lastErrorCode: string | null; candidateCount: number | null; completedCandidateCount: number | null; needsOperatorCandidateCount: number | null; failedCandidateCount: number | null };
...
function project(row: Row): AdminYoutubeCaptureDetail[] {
  ...
  const capture: AdminYoutubeCapture = { sourceId: row.sourceId, sourceLabel: safeText(row.sourceLabel, 500) ?? "YouTube video", displayUrl: safeUrl(row.sourceCanonicalUrl ?? row.sourceUrl), createdAt: row.createdAt.toISOString(), capturedAt: safeTimestamp(row.capturedAt), captureMethod: "gemini_youtube_url", evidenceCount: evidence.length, ingestionJob: row.jobId && row.jobStatus && row.jobUpdatedAt ? { status: row.jobStatus, updatedAt: row.jobUpdatedAt.toISOString(), lastErrorCode: row.lastErrorCode && /^[a-z0-9_:-]{1,120}$/.test(row.lastErrorCode) ? row.lastErrorCode : null, candidateCount: row.candidateCount ?? 0, completedCandidateCount: row.completedCandidateCount ?? 0, needsOperatorCandidateCount: row.needsOperatorCandidateCount ?? 0, failedCandidateCount: row.failedCandidateCount ?? 0 } : null };
