...
function validate(input: AdminAiGatewayModelInput | AdminAiGatewayModelUpdate) {
  ...
  if (input.defaultForPurpose && (input.purpose === "extraction" || input.purpose === "youtube_discovery_triage" || input.purpose === "youtube_discovery_province_suggestion") && (!input.supportsTextInput || !input.supportsExtraction)) throw new AdminAiModelCatalogPolicyError("Default extraction model must support text input and extraction.");
