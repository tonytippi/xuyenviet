...
export type YoutubeDiscoveryPolicyAuditSummary = Readonly<{
  ...
  candidateBacklogThreshold: number;
...
export type AdminYoutubeDiscoveryImmediateRunCommand = Readonly<{ confirmationKey: string }>;
export type AdminYoutubeDiscoveryImmediateRunResult = Readonly<{ runId: string; state: "queued" | "running" | "completed" | "failed" | "cancelled"; createdAt: string }>;
export type AdminYoutubeDiscoveryQueryProgress = Readonly<{ run: AdminYoutubeDiscoveryImmediateRunResult & { claimedAt: string | null; terminalAt: string | null; retryCount: number; nextRetryAt: string | null; safeErrorCode: "stage_transient" | "search_transient" | "search_timeout" | "enrichment_transient" | "execution_timeout" | "triage_transient" | "triage_timeout" | "eligibility_unavailable" | "recommendation_transient" | "persistence_contended" | "retry_exhausted" | "lease_retry_exhausted" | "policy_revoked" | null }; candidateCount: number; jobs: Readonly<{ queued: number; running: number; retrying: number; completed: number; failed: number; cancelled: number }>; reviewAvailable: boolean }>;
export function parseAdminYoutubeDiscoveryImmediateRunCommand(value: unknown): AdminYoutubeDiscoveryImmediateRunCommand | null { return record(value) && exactKeys(value, ["confirmationKey"]) && typeof value.confirmationKey === "string" && /^[A-Za-z0-9_-]{16,128}$/.test(value.confirmationKey) ? value as AdminYoutubeDiscoveryImmediateRunCommand : null; }
export function parseAdminYoutubeDiscoveryImmediateRunResult(value: unknown): AdminYoutubeDiscoveryImmediateRunResult | null { return record(value) && exactKeys(value, ["runId", "state", "createdAt"]) && identifier(value.runId) && ["queued", "running", "completed", "failed", "cancelled"].includes(value.state as string) && isoTimestamp(value.createdAt) ? value as AdminYoutubeDiscoveryImmediateRunResult : null; }
export function parseAdminYoutubeDiscoveryQueryProgress(value: unknown): AdminYoutubeDiscoveryQueryProgress | null { if (!record(value) || !exactKeys(value, ["run", "candidateCount", "jobs", "reviewAvailable"]) || !record(value.run) || !exactKeys(value.run, ["runId", "state", "createdAt", "claimedAt", "terminalAt", "retryCount", "nextRetryAt", "safeErrorCode"]) || !parseAdminYoutubeDiscoveryImmediateRunResult({ runId: value.run.runId, state: value.run.state, createdAt: value.run.createdAt }) || !(value.run.claimedAt === null || isoTimestamp(value.run.claimedAt)) || !(value.run.terminalAt === null || isoTimestamp(value.run.terminalAt)) || !nonNegativeInteger(value.run.retryCount) || (value.run.retryCount as number) > 11 || (value.run.nextRetryAt !== null && !isoTimestamp(value.run.nextRetryAt)) || (value.run.nextRetryAt === null && value.run.safeErrorCode !== null && value.run.state === "queued") || !(value.run.safeErrorCode === null || ["stage_transient", "search_transient", "search_timeout", "enrichment_transient", "execution_timeout", "triage_transient", "triage_timeout", "eligibility_unavailable", "recommendation_transient", "persistence_contended", "retry_exhausted", "lease_retry_exhausted", "policy_revoked"].includes(value.run.safeErrorCode as string)) || !nonNegativeInteger(value.candidateCount) || typeof value.reviewAvailable !== "boolean" || !record(value.jobs) || !exactKeys(value.jobs, ["queued", "running", "retrying", "completed", "failed", "cancelled"]) || ![value.jobs.queued, value.jobs.running, value.jobs.retrying, value.jobs.completed, value.jobs.failed, value.jobs.cancelled].every(nonNegativeInteger)) return null; return value as AdminYoutubeDiscoveryQueryProgress; }
...
export type AdminYoutubeDiscoveryQuality = Readonly<{ tooShort: number; durationUnknown: number; nonVietnamese: number; languageUnknown: number; foreignFallback: number; vietnameseConsider: number; considered: number; vietnameseFitPercent: number | null; durationViolations: number }>;
export type AdminYoutubeDiscoveryMissionFunnel = Readonly<{ asOf: string; discovered: number; enriched: number; triaged: number; recommended: number; pendingReview: number; accepted: number; deferred: number; skipped: number; quality: AdminYoutubeDiscoveryQuality }>;
...
export function parseAdminYoutubeDiscoveryMissionFunnel(value: unknown): AdminYoutubeDiscoveryMissionFunnel | null { return record(value) && exactKeys(value, ["asOf", "discovered", "enriched", "triaged", "recommended", "pendingReview", "accepted", "deferred", "skipped", "quality"]) && isoTimestamp(value.asOf) && [value.discovered, value.enriched, value.triaged, value.recommended, value.pendingReview, value.accepted, value.deferred, value.skipped].every(nonNegativeInteger) && quality(value.quality) ? value as AdminYoutubeDiscoveryMissionFunnel : null; }
...
export type AdminYoutubeDiscoveryReviewQueueItem = Readonly<{ recommendationId: string; canonicalUrl: string; thumbnailUrl: string | null; title: string | null; channelName: string | null; publishedAt: string | null; durationSeconds: number | null; viewCount: number | null; languageFit: "vi" | "likely_vi"; eligibilityReason: "eligible_vietnamese"; recommendation: ReviewRecommendation; reason: ReviewReason; actionAvailability: "available" | "reconciling" }>;
export type AdminYoutubeDiscoveryReviewQueue = Readonly<{ items: AdminYoutubeDiscoveryReviewQueueItem[]; nextCursor: string | null }>;
export type AdminYoutubeDiscoveryForeignFallbackItem = Readonly<{ canonicalUrl: string; thumbnailUrl: string | null; title: string | null; channelName: string | null; publishedAt: string | null; durationSeconds: number | null; viewCount: number | null; languageFit: "unknown" | "non_vi"; eligibilityReason: "foreign_fallback"; queryText: string }>;
export type AdminYoutubeDiscoveryForeignFallbackList = Readonly<{ items: AdminYoutubeDiscoveryForeignFallbackItem[] }>;
...
export function parseAdminYoutubeDiscoveryReviewQueueItem(value: unknown): AdminYoutubeDiscoveryReviewQueueItem | null {
  if (!record(value) || !exactKeys(value, ["recommendationId", "canonicalUrl", "thumbnailUrl", "title", "channelName", "publishedAt", "durationSeconds", "viewCount", "languageFit", "eligibilityReason", "recommendation", "reason", "actionAvailability"])) return null;
  const duration = value.durationSeconds;
  return identifier(value.recommendationId) && canonicalUrl(value.canonicalUrl) && nullableThumbnail(value.thumbnailUrl) && nullableText(value.title, 200) && nullableText(value.channelName, 160) && (value.publishedAt === null || isoTimestamp(value.publishedAt)) && (duration === null || typeof duration === "number" && Number.isSafeInteger(duration) && duration >= 0 && duration <= 86_400) && (value.viewCount === null || nonNegativeInteger(value.viewCount)) && (value.languageFit === "vi" || value.languageFit === "likely_vi") && value.eligibilityReason === "eligible_vietnamese" && value.recommendation === "consider" && value.reason === "eligible_score_band" && (value.actionAvailability === "available" || value.actionAvailability === "reconciling") ? value as AdminYoutubeDiscoveryReviewQueueItem : null;
}
...
export function parseAdminYoutubeDiscoveryForeignFallbackItem(value: unknown): AdminYoutubeDiscoveryForeignFallbackItem | null { return record(value) && exactKeys(value, ["canonicalUrl", "thumbnailUrl", "title", "channelName", "publishedAt", "durationSeconds", "viewCount", "languageFit", "eligibilityReason", "queryText"]) && canonicalUrl(value.canonicalUrl) && nullableThumbnail(value.thumbnailUrl) && nullableText(value.title, 200) && nullableText(value.channelName, 160) && (value.publishedAt === null || isoTimestamp(value.publishedAt)) && (value.durationSeconds === null || nonNegativeInteger(value.durationSeconds) && (value.durationSeconds as number) <= 86_400) && (value.viewCount === null || nonNegativeInteger(value.viewCount)) && (value.languageFit === "unknown" || value.languageFit === "non_vi") && value.eligibilityReason === "foreign_fallback" && safeQueryText(value.queryText) ? value as AdminYoutubeDiscoveryForeignFallbackItem : null; }
export function parseAdminYoutubeDiscoveryForeignFallbackList(value: unknown): AdminYoutubeDiscoveryForeignFallbackList | null { return record(value) && exactKeys(value, ["items"]) && Array.isArray(value.items) && value.items.length <= 20 && value.items.every((item) => parseAdminYoutubeDiscoveryForeignFallbackItem(item) !== null) ? value as AdminYoutubeDiscoveryForeignFallbackList : null; }
export function parseAdminYoutubeDiscoveryReviewDetail(value: unknown): AdminYoutubeDiscoveryReviewDetail | null {
  if (!record(value) || !exactKeys(value, ["recommendationId", "canonicalUrl", "thumbnailUrl", "title", "channelName", "publishedAt", "durationSeconds", "viewCount", "languageFit", "eligibilityReason", "recommendation", "reason", "actionAvailability", "queryText", "queryReason", "score", "factors", "penalties", "signals", "priorCaptureOutcome"]) || !parseAdminYoutubeDiscoveryReviewQueueItem({ recommendationId: value.recommendationId, canonicalUrl: value.canonicalUrl, thumbnailUrl: value.thumbnailUrl, title: value.title, channelName: value.channelName, publishedAt: value.publishedAt, durationSeconds: value.durationSeconds, viewCount: value.viewCount, languageFit: value.languageFit, eligibilityReason: value.eligibilityReason, recommendation: value.recommendation, reason: value.reason, actionAvailability: value.actionAvailability })) return null;
...
export const adminYoutubeDiscoveryBrowsePageSize = 20;
export type AdminYoutubeDiscoveryBrowseFilter = "all" | "skip" | "defer" | "consider";
export type AdminYoutubeDiscoveryBrowseCursor = Readonly<{ version: 1; filter: AdminYoutubeDiscoveryBrowseFilter; createdAt: string; recommendationId: string }>;
export type AdminYoutubeDiscoveryBrowseItem = Readonly<{ recommendationId: string; canonicalUrl: string; title: string | null; channelName: string | null; publishedAt: string | null; durationSeconds: number | null; recommendation: Exclude<AdminYoutubeDiscoveryBrowseFilter, "all">; reason: "eligible_score_band" | "below_defer_band" | "between_defer_and_consider_band" | "already_compatible" | "canonical_mismatch" | "not_current_run_enriched"; score: number; factors: ReviewFactor[]; penalties: ReviewPenalty[]; signals: ReviewSignal[]; createdAt: string }>;
export type AdminYoutubeDiscoveryBrowsePage = Readonly<{ items: AdminYoutubeDiscoveryBrowseItem[]; nextCursor: string | null }>;

export function encodeAdminYoutubeDiscoveryBrowseCursor(cursor: AdminYoutubeDiscoveryBrowseCursor): string { return browseCursor(cursor) ? `ydb1.${btoa(JSON.stringify(cursor)).replaceAll("+", "-").replaceAll("/", "_").replaceAll("=", "")}` : (() => { throw new Error("Invalid YouTube Discovery browse cursor."); })(); }
export function parseAdminYoutubeDiscoveryBrowseCursor(value: unknown): AdminYoutubeDiscoveryBrowseCursor | null { if (typeof value !== "string" || !/^ydb1\.[A-Za-z0-9_-]{1,512}$/.test(value)) return null; try { const encoded = value.slice(5).replaceAll("-", "+").replaceAll("_", "/"); const parsed: unknown = JSON.parse(atob(encoded + "=".repeat((4 - encoded.length % 4) % 4))); return browseCursor(parsed) ? parsed : null; } catch { return null; } }
export function parseAdminYoutubeDiscoveryBrowseItem(value: unknown): AdminYoutubeDiscoveryBrowseItem | null {
  if (!record(value) || !exactKeys(value, ["recommendationId", "canonicalUrl", "title", "channelName", "publishedAt", "durationSeconds", "recommendation", "reason", "score", "factors", "penalties", "signals", "createdAt"])) return null;
  return identifier(value.recommendationId) && canonicalUrl(value.canonicalUrl) && nullableText(value.title, 200) && nullableText(value.channelName, 160) && (value.publishedAt === null || isoTimestamp(value.publishedAt)) && (value.durationSeconds === null || Number.isSafeInteger(value.durationSeconds) && (value.durationSeconds as number) >= 0 && (value.durationSeconds as number) <= 86_400) && (value.recommendation === "skip" || value.recommendation === "defer" || value.recommendation === "consider") && ["eligible_score_band", "below_defer_band", "between_defer_and_consider_band", "already_compatible", "canonical_mismatch", "not_current_run_enriched"].includes(value.reason as string) && finiteScore(value.score) && codes(value.factors, ["relevance", "expected_value", "freshness_fit"], 3) && codes(value.penalties, ["commercial_risk", "duplicate_risk"], 2) && (value.factors as unknown[]).length + (value.penalties as unknown[]).length <= 5 && codes(value.signals, ["recent_discussion", "stale_or_changed_warning", "practical_question_demand", "creator_responsiveness", "commercial_risk", "contradictory_discussion"], 6) && isoTimestamp(value.createdAt) ? value as AdminYoutubeDiscoveryBrowseItem : null;
}
export function parseAdminYoutubeDiscoveryBrowsePage(value: unknown): AdminYoutubeDiscoveryBrowsePage | null { return record(value) && exactKeys(value, ["items", "nextCursor"]) && Array.isArray(value.items) && value.items.length <= adminYoutubeDiscoveryBrowsePageSize && value.items.every((item) => parseAdminYoutubeDiscoveryBrowseItem(item) !== null) && (value.nextCursor === null || parseAdminYoutubeDiscoveryBrowseCursor(value.nextCursor) !== null) ? value as AdminYoutubeDiscoveryBrowsePage : null; }
...
export type AdminYoutubeDiscoveryHealthOverview = Readonly<{ asOf: string; lastUpdatedAt: string | null; policy: AdminYoutubeDiscoveryHealthPolicy; planning: AdminYoutubeDiscoveryHealthRun; querySchedule: AdminYoutubeDiscoveryHealthSchedule; latestQueryRun: AdminYoutubeDiscoveryHealthRun; pausedRuns: AdminYoutubeDiscoveryPausedRun[]; throughput: Readonly<{ windowHours: number; discovered: number; enriched: number; triaged: number; recommended: number; lastUpdatedAt: string | null; freshness: "current" | "stale" | "unavailable" }>; backlog: Readonly<{ pending: number; deferred: number; candidateQueued: number; candidateRetrying: number; candidateRunning: number; oldestDeferredAt: string | null; deferredAge: "available" | "unavailable"; lastUpdatedAt: string | null }>; quality: AdminYoutubeDiscoveryQuality; incidents: AdminYoutubeDiscoveryActionRequiredItem[]; usage: Readonly<{ availability: "available" | "missing" | "incomplete_usage" | "incomplete_pricing"; requests: number; totalTokens: number | null; costMicros: number | null; lastUpdatedAt: string | null; freshness: "current" | "stale" | "unavailable" }> }>;
export type AdminYoutubeDiscoveryHealthIncidentCursor = Readonly<{ version: 1; groupId: string; at: string; executionId: string }>;
export type AdminYoutubeDiscoveryHealthIncidentItem =
  | Readonly<{ executionKind: "query_run"; runId: string; state: "retrying" | "failed" | "completed"; stage: "unavailable"; safeErrorCode: null; phase: "retrying" | "terminal" | "completed"; at: string; nextRunAt: string | null; retryCount: number; category: "provider_rate_limited" | "triage_schema_invalid" | "execution_terminal" }>
  | Readonly<{ executionKind: "candidate_job"; candidateJobId: string; state: "running"; stage: null; safeErrorCode: null; phase: "running"; at: string; nextRunAt: null; retryCount: number; category: "provider_rate_limited" | "triage_schema_invalid" | "execution_terminal" }>
  | Readonly<{ executionKind: "candidate_job"; candidateJobId: string; state: "retrying" | "failed"; stage: "enrichment" | "triage" | "eligibility" | "recommendation" | null; safeErrorCode: "stage_transient" | "enrichment_transient" | "triage_transient" | "triage_timeout" | "eligibility_unavailable" | "recommendation_transient" | "persistence_contended" | "retry_exhausted" | "lease_retry_exhausted"; phase: "retrying" | "terminal"; at: string; nextRunAt: string | null; retryCount: number; category: "provider_rate_limited" | "triage_schema_invalid" | "execution_terminal" }>;
...
export function parseAdminYoutubeDiscoveryHealthOverview(value: unknown): AdminYoutubeDiscoveryHealthOverview | null {
  if (!record(value) || !exactKeys(value, ["asOf", "lastUpdatedAt", "policy", "planning", "querySchedule", "latestQueryRun", "pausedRuns", "throughput", "backlog", "quality", "incidents", "usage"]) || !isoTimestamp(value.asOf) || (value.lastUpdatedAt !== null && !isoTimestamp(value.lastUpdatedAt)) || !healthPolicy(value.policy) || !healthRun(value.planning) || !healthSchedule(value.querySchedule) || !healthRun(value.latestQueryRun, value.policy.enabled === false) || !Array.isArray(value.pausedRuns) || value.pausedRuns.length > 20 || !value.pausedRuns.every(pausedRun) || !quality(value.quality)) return null;
  ...
  return record(throughput) && exactKeys(throughput, ["windowHours", "discovered", "enriched", "triaged", "recommended", "lastUpdatedAt", "freshness"]) && throughput.windowHours === adminYoutubeDiscoveryHealthStageWindowHours && [throughput.discovered, throughput.enriched, throughput.triaged, throughput.recommended].every(nonNegativeInteger) && (throughput.lastUpdatedAt === null || isoTimestamp(throughput.lastUpdatedAt)) && ["current", "stale", "unavailable"].includes(throughput.freshness as string) && (throughput.lastUpdatedAt === null ? throughput.freshness === "unavailable" : throughput.freshness !== "unavailable")
    && record(backlog) && exactKeys(backlog, ["pending", "deferred", "candidateQueued", "candidateRetrying", "candidateRunning", "oldestDeferredAt", "deferredAge", "lastUpdatedAt"]) && nonNegativeInteger(backlog.pending) && nonNegativeInteger(backlog.deferred) && nonNegativeInteger(backlog.candidateQueued) && nonNegativeInteger(backlog.candidateRetrying) && nonNegativeInteger(backlog.candidateRunning) && (backlog.oldestDeferredAt === null || isoTimestamp(backlog.oldestDeferredAt)) && (backlog.lastUpdatedAt === null || isoTimestamp(backlog.lastUpdatedAt)) && (backlog.deferredAge === "available" || backlog.deferredAge === "unavailable") && (backlog.deferredAge === "available" ? (backlog.deferred as number) > 0 && backlog.oldestDeferredAt !== null : backlog.oldestDeferredAt === null)
...
function nullableThumbnail(value: unknown): boolean { return value === null || typeof value === "string" && /^https:\/\/(i\.ytimg\.com|img\.youtube\.com)\/[^\s]{1,500}$/.test(value); }
...
function browseCursor(value: unknown): value is AdminYoutubeDiscoveryBrowseCursor { return record(value) && exactKeys(value, ["version", "filter", "createdAt", "recommendationId"]) && value.version === 1 && (value.filter === "all" || value.filter === "skip" || value.filter === "defer" || value.filter === "consider") && reviewCursorTimestamp(value.createdAt) && identifier(value.recommendationId); }
...
function quality(value: unknown): value is AdminYoutubeDiscoveryQuality { return record(value) && exactKeys(value, ["tooShort", "durationUnknown", "nonVietnamese", "languageUnknown", "foreignFallback", "vietnameseConsider", "considered", "vietnameseFitPercent", "durationViolations"]) && [value.tooShort, value.durationUnknown, value.nonVietnamese, value.languageUnknown, value.foreignFallback, value.vietnameseConsider, value.considered, value.durationViolations].every(nonNegativeInteger) && (value.vietnameseFitPercent === null || typeof value.vietnameseFitPercent === "number" && Number.isFinite(value.vietnameseFitPercent) && value.vietnameseFitPercent >= 0 && value.vietnameseFitPercent <= 100) && (value.considered === 0 ? value.vietnameseFitPercent === null : value.vietnameseFitPercent === Math.round((value.vietnameseConsider as number) / (value.considered as number) * 10_000) / 100) && (value.vietnameseConsider as number) <= (value.considered as number); }
...
function healthIncidentCursor(value: unknown): value is AdminYoutubeDiscoveryHealthIncidentCursor { return record(value) && exactKeys(value, ["version", "groupId", "at", "executionId"]) && value.version === 1 && incidentGroup(value.groupId) && reviewCursorTimestamp(value.at) && identifier(value.executionId); }
function healthIncidentItem(value: unknown): value is AdminYoutubeDiscoveryHealthIncidentItem {
  if (!record(value) || !isoTimestamp(value.at) || !nonNegativeInteger(value.retryCount) || (value.retryCount as number) > 10 || !incidentCategory(value.category)) return false;
  if (value.executionKind === "query_run") return exactKeys(value, ["executionKind", "runId", "state", "stage", "safeErrorCode", "phase", "at", "nextRunAt", "retryCount", "category"]) && identifier(value.runId) && value.stage === "unavailable" && value.safeErrorCode === null && ((value.state === "retrying" && value.phase === "retrying" && isoTimestamp(value.nextRunAt)) || (value.state === "failed" && value.phase === "terminal" && value.nextRunAt === null) || (value.state === "completed" && value.phase === "completed" && value.nextRunAt === null));
  if (value.executionKind !== "candidate_job" || !exactKeys(value, ["executionKind", "candidateJobId", "state", "stage", "safeErrorCode", "phase", "at", "nextRunAt", "retryCount", "category"]) || !identifier(value.candidateJobId)) return false;
  if (value.state === "running") return value.stage === null && value.safeErrorCode === null && value.phase === "running" && value.nextRunAt === null;
  return (value.state === "retrying" || value.state === "failed") && (value.stage === "enrichment" || value.stage === "triage" || value.stage === "eligibility" || value.stage === "recommendation" || value.stage === null) && typeof value.safeErrorCode === "string" && ["stage_transient", "enrichment_transient", "triage_transient", "triage_timeout", "eligibility_unavailable", "recommendation_transient", "persistence_contended", "retry_exhausted", "lease_retry_exhausted"].includes(value.safeErrorCode) && ((value.state === "retrying" && value.phase === "retrying" && isoTimestamp(value.nextRunAt)) || (value.state === "failed" && value.phase === "terminal" && value.nextRunAt === null));
}
...
function provinceId(value: unknown): value is string { return typeof value === "string" && (governedKnowledgeProvinceIds as readonly string[]).includes(value); }
function requiredText(value: unknown, maximum: number): value is string { return typeof value === "string" && value.trim() === value && value.length > 0 && value.length <= maximum && !/[\x00-\x1F\x7F]/.test(value); }
function vietnameseText(value: unknown, maximum: number): value is string { return requiredText(value, maximum) && /[\u00C0-\u1EF9đĐ]/.test(value); }
