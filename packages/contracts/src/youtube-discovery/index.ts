...
export function parseAdminYoutubeDiscoveryHealthOverview(value: unknown): AdminYoutubeDiscoveryHealthOverview | null {
  if (!record(value) || !exactKeys(value, ["asOf", "lastUpdatedAt", "policy", "planning", "querySchedule", "latestQueryRun", "throughput", "backlog", "incidents", "usage"]) || !isoTimestamp(value.asOf) || (value.lastUpdatedAt !== null && !isoTimestamp(value.lastUpdatedAt)) || !healthPolicy(value.policy) || !healthRun(value.planning) || !healthSchedule(value.querySchedule) || !healthRun(value.latestQueryRun)) return null;
