...
export type AdminFacebookCaptureJob = { status: "queued" | "running" | "completed" | "failed"; discoveryTerminal: boolean; updatedAt: string; lastErrorCode: string | null; candidateCount: number; completedCandidateCount: number; needsOperatorCandidateCount: number; failedCandidateCount: number };
...
function isAdminFacebookCapture(value: unknown): value is AdminFacebookCapture {
  ...
  return job === null || isRecord(job) && hasExactKeys(job, ["status", "discoveryTerminal", "updatedAt", "lastErrorCode", "candidateCount", "completedCandidateCount", "needsOperatorCandidateCount", "failedCandidateCount"]) && ["queued", "running", "completed", "failed"].includes(job.status as string) && typeof job.discoveryTerminal === "boolean" && typeof job.updatedAt === "string" && (typeof job.lastErrorCode === "string" || job.lastErrorCode === null) && [job.candidateCount, job.completedCandidateCount, job.needsOperatorCandidateCount, job.failedCandidateCount].every((count) => Number.isInteger(count) && (count as number) >= 0);
}
...
export type AdminYoutubeCaptureIngestionJob = { status: "queued" | "running" | "completed" | "failed"; updatedAt: string };
...
function isAdminYoutubeCapture(value: unknown): value is AdminYoutubeCapture {
  ...
  const job = value.ingestionJob;
  return job === null || isRecord(job) && hasExactKeys(job, ["status", "updatedAt"]) && ["queued", "running", "completed", "failed"].includes(job.status as string) && isoTimestamp(job.updatedAt);
}
...
export function parseAdminKnowledgeApprovedList(value: unknown) { return isRecord(value) && hasExactKeys(value, ["candidateCount", "items"]) && Number.isInteger(value.candidateCount) && (value.candidateCount as number) >= 0 && Array.isArray(value.items) && value.items.length <= 200 && value.items.every((item) => safeDraft({ ...item, suggestion: null })) ? value as { candidateCount: number; items: Array<Omit<AdminKnowledgeDraft, "suggestion"> & { indexStatus: { state: string; label: string; documentStatus: string | null; indexedAt: string | null } | null }> } : null; }
...
function safeRecommendation(value: unknown) { if (!isRecord(value) || !hasExactKeys(value, ["id", "status", "resolution", "workType", "priority", "contentVersion", "evidenceSetRevision", "createdAt", "card"]) || !isRecord(value.card)) return false; const card = value.card; return reviewId(value.id) && safeString(value.status, 80) && safeString(value.resolution, 80, true) && ["risk", "missing_context", "verification", "relation", "sampling"].includes(value.workType as string) && Number.isInteger(value.priority) && (value.priority as number) >= 1 && (value.priority as number) <= 100 && Number.isInteger(value.contentVersion) && (value.contentVersion as number) >= 1 && Number.isInteger(value.evidenceSetRevision) && (value.evidenceSetRevision as number) >= 1 && isoTimestamp(value.createdAt) && ["id", "title", "summary", "lifecycleState", "knowledgeState", "verificationRequirement", "type", "locationName", "routeSegment", "tags", "freshnessSensitive"].every((key) => key in card) && reviewId(card.id) && safeString(card.title, 160) && safeString(card.summary, 1200); }
...
export function parseAdminKnowledgeRecommendationDetail(value: unknown) { if (!safeRecommendation(value) || !isRecord(value) || !hasExactKeys(value, ["id", "status", "resolution", "workType", "priority", "contentVersion", "evidenceSetRevision", "createdAt", "card", "evidence"]) || !Array.isArray(value.evidence) || value.evidence.length > 4) return null; return value.evidence.every((item: unknown) => isRecord(item) && hasExactKeys(item, ["quote", "conditions", "supportLevel", "displayPolicy", "capturedAt", "sourceLabel", "sourceKind", "facebookReviewId"]) && safeString(item.quote, 500) && Array.isArray(item.conditions) && item.conditions.length <= 10 && item.conditions.every((condition: unknown) => safeString(condition, 160)) && safeString(item.supportLevel, 80) && safeString(item.displayPolicy, 80) && isoTimestamp(item.capturedAt) && safeString(item.sourceLabel, 160) && safeString(item.sourceKind, 80) && (item.facebookReviewId === null || reviewId(item.facebookReviewId))) ? value : null; }
export function parseAdminKnowledgeRecommendationResolve(value: unknown) { if (!isRecord(value) || !["expectedContentVersion", "expectedEvidenceSetRevision", "action"].every((key) => key in value) || Object.keys(value).some((key) => !["expectedContentVersion", "expectedEvidenceSetRevision", "action", "editSummary"].includes(key))) return null; return Number.isInteger(value.expectedContentVersion) && (value.expectedContentVersion as number) >= 1 && Number.isInteger(value.expectedEvidenceSetRevision) && (value.expectedEvidenceSetRevision as number) >= 1 && ["accept_wording", "edit", "suppress", "restore", "verify", "promote", "resolve_relation", "sampling_pass", "sampling_fail"].includes(value.action as string) && (value.editSummary === undefined || safeString(value.editSummary, 1200)) ? value : null; }
