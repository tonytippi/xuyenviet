...
export type AdminFacebookCaptureCandidate = { processingStatus: "queued" | "processing" | "completed" | "failed"; aiDisposition: "apply" | "needs_operator" | "discard" | null; outcomeReasonCode: string | null; card: AdminKnowledgeOperatorCard | null };
...
export type AdminFacebookCaptureDetail = AdminFacebookCapture & { candidates: AdminFacebookCaptureCandidate[]; canRecapture: boolean; canRerunIngestion: boolean };
...
function safeCard(value: unknown): value is AdminKnowledgeCard { return isRecord(value) && hasExactKeys(value, ["id", "type", "title", "locationName", "routeSegment", "summary", "practicalDetails", "tags", "confidence", "freshnessSensitive", "lifecycleState", "knowledgeState", "verificationRequirement", "updatedAt", "createdAt", "sources", "indexStatus"]) && reviewId(value.id) && safeString(value.type, 80) && safeString(value.title, 160) && safeString(value.locationName, 160, true) && safeString(value.routeSegment, 160, true) && safeString(value.summary, 1200) && safeDetails(value.practicalDetails) && Array.isArray(value.tags) && value.tags.length <= 12 && value.tags.every((tag) => safeString(tag, 40)) && safeString(value.confidence, 80) && typeof value.freshnessSensitive === "boolean" && ["draft", "pending_operator", "active", "suppressed", "archived", "rejected"].includes(value.lifecycleState as string) && ["community_observation", "community_pattern", "conditional", "conflicted"].includes(value.knowledgeState as string) && ["none", "operator_required", "failed"].includes(value.verificationRequirement as string) && isoTimestamp(value.updatedAt) && isoTimestamp(value.createdAt) && Array.isArray(value.sources) && value.sources.every(safeSource) && (value.indexStatus === null || isRecord(value.indexStatus) && hasExactKeys(value.indexStatus, ["state", "label", "documentStatus", "indexedAt"]) && safeString(value.indexStatus.state, 80) && safeString(value.indexStatus.label, 160) && safeString(value.indexStatus.documentStatus, 80, true) && safeString(value.indexStatus.indexedAt, 100, true) && (value.indexStatus.indexedAt === null || isoTimestamp(value.indexStatus.indexedAt))); }
export function parseAdminKnowledgeCardListQuery(value: unknown) { return isRecord(value) && Object.keys(value).every((key) => key === "lifecycleState" || key === "q") && (value.lifecycleState === undefined || ["draft", "pending_operator", "active", "suppressed", "archived", "rejected"].includes(value.lifecycleState as string)) && (value.q === undefined || safeString(value.q, 240)) ? { lifecycleState: value.lifecycleState as AdminKnowledgeCard["lifecycleState"] | undefined, q: value.q as string | undefined } : null; }
