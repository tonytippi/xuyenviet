...
export function parseAdminKnowledgeRecommendationResolve(value: unknown) { if (!isRecord(value) || !["expectedContentVersion", "expectedEvidenceSetRevision", "action"].every((key) => key in value) || Object.keys(value).some((key) => !["expectedContentVersion", "expectedEvidenceSetRevision", "action", "editSummary", "highSeverity"].includes(key))) return null; return Number.isInteger(value.expectedContentVersion) && (value.expectedContentVersion as number) >= 1 && Number.isInteger(value.expectedEvidenceSetRevision) && (value.expectedEvidenceSetRevision as number) >= 1 && ["accept_wording", "edit", "suppress", "restore", "verify", "promote", "resolve_relation", "sampling_pass", "sampling_fail"].includes(value.action as string) && (value.editSummary === undefined || safeString(value.editSummary, 1200)) && (value.highSeverity === undefined || typeof value.highSeverity === "boolean") && (!(value.highSeverity === true) || value.action === "sampling_fail") ? value : null; }
