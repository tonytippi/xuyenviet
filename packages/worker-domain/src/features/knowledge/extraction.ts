...
export async function extractKnowledgeDraftsFromSourceAsActor(sourceId: string, actor: { userId: string; email: string }, options: { preProviderGuard?: KnowledgeDraftExtractionPreProviderGuard; resultJobId?: string; captureVersionId?: string | null; executorSystem?: SystemAuditActorId } = {}): Promise<KnowledgeDraftExtractionResult> {
