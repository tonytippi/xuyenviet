...
export type KnowledgeIngestionPipelineResult = { jobId: string; sourceId: string; outcome: "published" | "suppressed" | "review_recommended" | "verify_first" | "failed" | "retry"; cardId?: string };
...
async function finish(claim: KnowledgeIngestionClaim, stage: typeof knowledgeIngestionJobs.$inferSelect.stage, version: number, outcome: Exclude<KnowledgeIngestionPipelineResult["outcome"], "published" | "retry">, code: string, db: PipelineDb) {
...
async function retryOrFail(claim: KnowledgeIngestionClaim, stage: typeof knowledgeIngestionJobs.$inferSelect.stage, version: number, code: string, db: PipelineDb) {
  ...
  if (retried?.stage === "failed") return { jobId: claim.jobId, sourceId: claim.sourceId, outcome: "failed" as const };
  if (retried) return { jobId: claim.jobId, sourceId: claim.sourceId, outcome: "retry" as const };
