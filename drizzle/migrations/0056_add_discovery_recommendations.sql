...
CREATE TABLE "youtube_discovery_recommendations" (
  ...
  CONSTRAINT "youtube_discovery_recommendations_codes_check" CHECK ("recommendation" in ('skip','defer','consider') and "factors" <@ array['relevance','expected_value','freshness_fit']::text[] and youtube_discovery_text_array_unique("factors") and "penalties" <@ array['commercial_risk','duplicate_risk']::text[] and youtube_discovery_text_array_unique("penalties") and cardinality("factors") + cardinality("penalties") <= 5 and "reason" in ('eligible_score_band','below_defer_band','between_defer_and_consider_band','already_compatible','canonical_mismatch','not_current_run_enriched') and "signals" <@ array['recent_discussion','stale_or_changed_warning','practical_question_demand','creator_responsiveness','commercial_risk','contradictory_discussion']::text[] and youtube_discovery_text_array_unique("signals") and cardinality("signals") <= 6)
...
CREATE UNIQUE INDEX "youtube_discovery_recommendations_id_provenance_idx" ON "youtube_discovery_recommendations" ("id", "candidate_id", "appearance_id", "run_id", "policy_version_id");--> statement-breakpoint
...
ALTER TABLE "youtube_discovery_ranking_history" ADD CONSTRAINT "youtube_discovery_ranking_history_recommendation_fk" FOREIGN KEY ("recommendation_id", "candidate_id", "appearance_id", "run_id", "policy_version_id") REFERENCES "youtube_discovery_recommendations"("id", "candidate_id", "appearance_id", "run_id", "policy_version_id") ON DELETE cascade;--> statement-breakpoint
...
CREATE TRIGGER youtube_discovery_recommendation_immutable_trigger BEFORE UPDATE OR DELETE ON "youtube_discovery_recommendations" FOR EACH ROW EXECUTE FUNCTION youtube_discovery_recommendation_immutable();--> statement-breakpoint
