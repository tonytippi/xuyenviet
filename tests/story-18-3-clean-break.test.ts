...
describe("Story 18.3 migration 0047 clean break", () => {
  test("does not project schedules when the current Discovery policy is disabled", async () => {
    ...
    expect(migration).toMatch(/"safe_signal_summary" = "reason"/);
    try {
      ...
      await sql`create temporary table legacy_policies (is_current boolean not null, enabled boolean not null)`;
      await sql`insert into legacy_policies values (true, false)`;
      await sql`insert into legacy_proposals values ('legacy-system-request', 'system', 'operator_request', 50, 'Da Lat route', true, 60)`;
      await sql`insert into legacy_proposals values ('enabled-legacy-system', 'system', 'coverage_gap', 50, 'Da Lat route', true, 60)`;
      ...
      await sql.unsafe(migration.match(/UPDATE "youtube_discovery_query_proposals" SET "target_digest"[^;]+WHERE "origin" = 'system';/)![0].replaceAll('"youtube_discovery_query_proposals"', 'legacy_proposals').replaceAll('"youtube_discovery_policy_versions"', 'legacy_policies'));
      await sql.unsafe(migration.match(/UPDATE "youtube_discovery_query_proposals" SET "schedule_anchor_at" = CASE WHEN "enabled"[^;]+;/)![0].replaceAll('"youtube_discovery_query_proposals"', 'legacy_proposals').replaceAll('"youtube_discovery_policy_versions"', 'legacy_policies'));
      await expect(sql`alter table legacy_proposals add constraint legacy_system_reason_check check ((origin = 'system' and reason in ('coverage_gap', 'freshness_risk', 'unresolved_conflict', 'anonymized_demand') and safe_signal_summary = reason) or (origin = 'operator' and target_digest is null and safe_signal_summary is null))`).resolves.toBeDefined();
      ...
      await expect(sql`select origin, enabled, target_digest, safe_signal_summary, schedule_anchor_at, next_due_at from legacy_proposals where id = 'enabled-legacy-system'`).resolves.toEqual([{ origin: 'system', enabled: true, target_digest: expect.stringMatching(/^[a-f0-9]{64}$/), safe_signal_summary: 'coverage_gap', schedule_anchor_at: null, next_due_at: null }]);
    } finally {
      await sql.end();
    }
  });
