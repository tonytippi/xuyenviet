...
describe("admin YouTube Discovery contract", () => {
  ...
  test("requires exact bounded review projections and an opaque versioned cursor", () => {
    const item = { recommendationId: "recommendation-1", canonicalUrl: "https://www.youtube.com/watch?v=abcDEF12345", thumbnailUrl: "https://i.ytimg.com/vi/abcDEF12345/mqdefault.jpg", title: "Da Lat route", channelName: "Route channel", publishedAt: "2026-08-07T00:00:00.000Z", durationSeconds: 120, viewCount: 1000, languageFit: "vi" as const, eligibilityReason: "eligible_vietnamese" as const, recommendation: "consider" as const, reason: "eligible_score_band" as const, actionAvailability: "available" as const };
    ...
    expect(parseAdminYoutubeDiscoveryReviewQueue({ items: [{ ...item, languageFit: "unknown" }], nextCursor: null })).toBeNull();
    const fallback = { canonicalUrl: item.canonicalUrl, thumbnailUrl: item.thumbnailUrl, title: item.title, channelName: item.channelName, publishedAt: item.publishedAt, durationSeconds: item.durationSeconds, viewCount: item.viewCount, languageFit: "non_vi" as const, eligibilityReason: "foreign_fallback" as const, queryText: "Đà Lạt kinh nghiệm" };
    expect(parseAdminYoutubeDiscoveryForeignFallbackList({ items: [fallback] })).toEqual({ items: [fallback] });
    expect(parseAdminYoutubeDiscoveryForeignFallbackList({ items: [{ ...fallback, recommendationId: "unsafe" }] })).toBeNull();
  ...
  test("requires endpoint-specific Mission pages, current-state funnel, and unavailable safe context", () => {
    ...
    const quality = { tooShort: 1, durationUnknown: 1, nonVietnamese: 1, languageUnknown: 1, foreignFallback: 1, vietnameseConsider: 8, considered: 10, vietnameseFitPercent: 80, durationViolations: 0 };
    expect(parseAdminYoutubeDiscoveryMissionFunnel({ asOf: coverage.createdAt, discovered: 1, enriched: 1, triaged: 1, recommended: 1, pendingReview: 1, accepted: 0, deferred: 0, skipped: 0, quality })).not.toBeNull();
    expect(parseAdminYoutubeDiscoveryMissionFunnel({ asOf: coverage.createdAt, discovered: -1, enriched: 0, triaged: 0, recommended: 0, pendingReview: 0, accepted: 0, deferred: 0, skipped: 0, quality })).toBeNull();
    for (const unsafeKey of ["rawComment", "sourceMaterial", "prompt", "providerDiagnostics", "apiKey", "captureVersionId", "evidenceSpan", "travelerContent"]) {
  ...
  test("requires bounded Health projections and server-admitted opaque incident cursors", () => {
    ...
    expect(parseAdminYoutubeDiscoveryHealthOverview({ ...overview, backlog: { ...overview.backlog, candidateRunning: -1 } })).toBeNull();
    expect(parseAdminYoutubeDiscoveryHealthOverview({ ...overview, backlog: { pending: 1, deferred: 1, oldestDeferredAt: "2026-08-06T00:00:00.000Z", deferredAge: "available", lastUpdatedAt: "2026-08-06T00:00:00.000Z" } })).toBeNull();
    ...
    const cursor = encodeAdminYoutubeDiscoveryHealthIncidentCursor({ version: 1, groupId: overview.incidents[0].actionId, at: cursorAt, executionId: "run-1" });
    expect(parseAdminYoutubeDiscoveryHealthIncidentCursor(cursor)).toEqual({ version: 1, groupId: overview.incidents[0].actionId, at: cursorAt, executionId: "run-1" });
    expect(() => encodeAdminYoutubeDiscoveryHealthIncidentCursor({ version: 1, groupId: overview.incidents[0].actionId, at: overview.asOf, executionId: "run-1" })).toThrow("Invalid YouTube Discovery Mission cursor.");
    expect(parseAdminYoutubeDiscoveryHealthIncidentDetail({ groupId: overview.incidents[0].actionId, category: "provider_rate_limited", items: [{ executionKind: "query_run", runId: "run-1", state: "retrying", stage: "unavailable", safeErrorCode: null, phase: "retrying", at: overview.asOf, nextRunAt: "2026-08-07T00:15:00.000Z", retryCount: 1, category: "provider_rate_limited" }], nextCursor: cursor })).not.toBeNull();
    expect(parseAdminYoutubeDiscoveryHealthIncidentDetail({ groupId: "123e4567-e89b-12d3-a456-426614174000:triage_schema_invalid", category: "triage_schema_invalid", items: [{ executionKind: "candidate_job", candidateJobId: "job-1", state: "failed", stage: "triage", safeErrorCode: "triage_transient", phase: "terminal", at: overview.asOf, nextRunAt: null, retryCount: 1, category: "triage_schema_invalid" }], nextCursor: null })).not.toBeNull();
    expect(parseAdminYoutubeDiscoveryHealthIncidentDetail({ groupId: "123e4567-e89b-12d3-a456-426614174000:triage_schema_invalid", category: "triage_schema_invalid", items: [{ executionKind: "candidate_job", candidateJobId: "job-1", state: "failed", stage: "triage", safeErrorCode: "unsafe_error", phase: "terminal", at: overview.asOf, nextRunAt: null, retryCount: 1, category: "triage_schema_invalid" }], nextCursor: null })).toBeNull();
    expect(parseAdminYoutubeDiscoveryHealthIncidentDetail({ groupId: overview.incidents[0].actionId, category: "provider_rate_limited", items: [{ executionKind: "query_run", runId: "run-1", state: "failed", stage: "unavailable", safeErrorCode: null, phase: "retrying", at: overview.asOf, nextRunAt: null, retryCount: 1, category: "provider_rate_limited" }], nextCursor: null })).toBeNull();
    expect(parseAdminYoutubeDiscoveryHealthIncidentDetail({ groupId: overview.incidents[0].actionId, category: "provider_rate_limited", items: [{ executionKind: "candidate_job", candidateJobId: "job-1", state: "retrying", stage: "triage", safeErrorCode: "triage_transient", phase: "retrying", at: overview.asOf, nextRunAt: null, retryCount: 1, category: "triage_schema_invalid" }], nextCursor: null })).toBeNull();
    expect(parseAdminYoutubeDiscoveryHealthIncidentDetail({ groupId: overview.incidents[0].actionId, category: "provider_rate_limited", items: [{ executionKind: "query_run", runId: "run-1", state: "retrying", phase: "retrying", at: overview.asOf, nextRunAt: "2026-08-07T00:15:00.000Z", retryCount: 1, category: "provider_rate_limited" }], nextCursor: null })).toBeNull();
    for (const unsafeKey of ["rawComment", "sourceMaterial", "prompt", "providerDiagnostics", "apiKey", "captureVersionId", "evidenceSpan", "travelerContent"]) {
