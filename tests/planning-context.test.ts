...
const session = {
  ...
  slotSourceMessageIds: { origin: "message-1", destination: "message-1" },
...
describe("planning context session contract", () => {
  ...
  test("rejects unknown, nested, workflow, reasoning, and provider data", () => {
    ...
    expect(parsePlanningContextSession({ ...session, slotSourceMessageIds: { origin: "message-1", destination: "unknown-slot" } })).toBeNull();
...
describe("planning clarification reducer", () => {
  test("collects one deterministic material value at a time and becomes ready", () => {
    ...
    if (complete.kind !== "ready") throw new Error("Expected ready session");
    expect(reducePlanningClarification({ session: complete.session, question: "Xuất phát từ Hà Nội", sourceMessageId: "message-5" })).toMatchObject({ kind: "ready", session: { revision: 5, slotSourceMessageIds: { origin: "message-5" } } });
  });

  test("keeps contradictory scope missing and supersedes a changed destination", () => {
    const collecting = { intent: "trip_planning" as const, slots: { origin: "Hà Nội", destination: "Đà Nẵng", start_date: "2026-12-20", adults: "2" }, slotSourceMessageIds: { origin: "message-1", destination: "message-1", start_date: "message-1", adults: "message-1" }, missingSlots: [], status: "ready" as const, sourceMessageIds: ["message-1"], revision: 1 };
  ...
  test("does not persist arbitrary text, invalid dates, or invalid adult counts", () => {
    const collecting = { intent: "trip_planning" as const, slots: { destination: "Đà Nẵng" }, slotSourceMessageIds: { destination: "message-1" }, missingSlots: ["origin", "start_date", "adults"] as Array<"origin" | "start_date" | "adults">, status: "collecting" as const, sourceMessageIds: ["message-1"], revision: 1 };
