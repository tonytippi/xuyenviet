...
describe.sequential("Knowledge province geography persistence", () => {
  ...
  test("migration backfills only geography fields for an existing exact legacy card", async () => {
    ...
    try {
      ...
      const installedReferences = await testDb.execute(sql.raw(`select id, display_name as "displayName", current_unit_id as "currentUnitId" from ${schemaName}.knowledge_province_references order by id`));
      expect(installedReferences).toEqual([...knowledgeProvinceReferenceFixture].sort((left, right) => left.id.localeCompare(right.id)).map(({ id, displayName, currentUnitId }) => ({ id, displayName, currentUnitId })));
