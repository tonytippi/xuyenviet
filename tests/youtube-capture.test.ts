...
describe("YouTube capture", () => {
  ...
  test("invalidates linked search projections when a YouTube title changes", async () => {
    ...
    await testDb.insert(knowledgeCardSearchDocuments).values({ knowledgeCardId: "linked-card", contentVersion: 1, acceptedFence: "legacy", executorSystem: "system-knowledge-pipeline", status: "active", searchableText: "YouTube video", textHash: "b".repeat(64), sourceCount: 1, confidence: "community", freshnessSensitive: false });
  ...
  test("rejects oversized cache strings instead of truncating evidence", async () => {
    await expect(admitArtifact(testDb as never, { provider: "youtube", reuseKey: "oversized", resourceIdentity: "video:abcDEF12345", captureMethodVersion: "test", payloadSchemaVersion: "test", promptVersion: "test", model: "test", payload: { evidence: "x".repeat(20_001) }, metadata: {}, capturedAt: "2026-08-05T00:00:00.000Z" })).rejects.toThrow("capture_artifact_payload_too_large");
  });
});
