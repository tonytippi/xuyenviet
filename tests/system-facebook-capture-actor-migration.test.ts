...
const reservedId = "system-facebook-capture";
const reservedEmail = "system-facebook-capture@xuyenviet.invalid";

describe("system Facebook capture actor migration", () => {
  let migration: string;

  beforeEach(async () => {
    await resetTestDatabase();
    migration = await readFile("drizzle/migrations/0065_system_facebook_capture_actor.sql", "utf8");
  });

  async function executeInIsolatedSchema(users: Array<{ id: string; email: string }>) {
    const schemaName = `migration_0065_${randomUUID().replaceAll("-", "")}`;
  ...
  test.each([
    ...
    await expect(executeInIsolatedSchema(users)).rejects.toThrow("Reserved system Facebook capture identity collides");
  });
});
