...
describe("Facebook visible-DOM capture", () => {
  test("selects visible DOM text without a network candidate path", () => {
    ...
    })).toMatchObject({ source: "innerText", text: "Hôm nay mình ẽ chia ẻ tiếp về phố cổ Hội An." });
  });
  ...
  test("does not select a slightly larger hidden text value", () => {
    const visible = "Hôm nay mình sẽ chia sẻ tiếp về phố cổ Hội An.";
    expect(chooseFacebookCaptureText({ innerText: visible, textContent: `${visible} Hidden menu.` })).toMatchObject({ source: "innerText", text: visible });
  });
