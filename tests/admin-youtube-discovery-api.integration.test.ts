...
const reviewItem = { recommendationId: "recommendation-1", canonicalUrl: "https://www.youtube.com/watch?v=abcDEF12345", thumbnailUrl: "https://i.ytimg.com/vi/abcDEF12345/mqdefault.jpg", title: "Da Lat route", channelName: "Route channel", publishedAt: "2026-08-07T00:00:00.000Z", durationSeconds: 120, viewCount: 100, languageFit: "vi" as const, eligibilityReason: "eligible_vietnamese" as const, recommendation: "consider" as const, reason: "eligible_score_band" as const, actionAvailability: "available" as const };
...
describe("admin YouTube Discovery direct API", () => {
  test("admits exact protected province coverage and fails closed before unsafe port data reaches the Mission", async () => {
    ...
    expect(port.listProvinceCoverage).not.toHaveBeenCalled();
    const operator = await browserSession("operator", "operator");
    port.listProvinceCoverage.mockResolvedValueOnce(valid);
    await request(app.getHttpServer()).get("/v1/admin/knowledge/youtube-discovery/province-coverage").set({ Cookie: operator.cookie }).expect(200, valid);
    expect(port.listProvinceCoverage).toHaveBeenCalledOnce();
    await request(app.getHttpServer()).get("/v1/admin/knowledge/youtube-discovery/province-coverage?unsafe=yes").set({ Cookie: operator.cookie }).expect(400);
    expect(port.listProvinceCoverage).toHaveBeenCalledOnce();
  ...
  test("admits the protected read-only foreign fallback projection", async () => {
    const operator = await browserSession("operator", "operator");
    const fallback = { canonicalUrl: "https://www.youtube.com/watch?v=abcDEF12345", thumbnailUrl: "https://i.ytimg.com/vi/abcDEF12345/mqdefault.jpg", title: "Foreign route", channelName: "Route channel", publishedAt: "2026-08-07T00:00:00.000Z", durationSeconds: 180, viewCount: 100, languageFit: "non_vi", eligibilityReason: "foreign_fallback", queryText: "Đà Lạt kinh nghiệm" };
    port.listForeignFallback.mockResolvedValueOnce({ items: [fallback] });
    await request(app.getHttpServer()).get("/v1/admin/knowledge/youtube-discovery/fallback").set({ Cookie: operator.cookie }).expect(200, { items: [fallback] });
    expect(port.listForeignFallback).toHaveBeenCalledOnce();
    await request(app.getHttpServer()).get("/v1/admin/knowledge/youtube-discovery/fallback?unsafe=yes").set({ Cookie: operator.cookie }).expect(400);
  });
